...
foreach ($locale in $locales) {
  ...
  if ($homeHtml -notmatch "data-locale=`"$([regex]::Escape($locale.code))`"") {
    Fail "Generated home page for '$($locale.code)' has incorrect Firebase chat locale."
  }
  if ($homeHtml -notmatch 'chat/chat-app\.js') {
    Fail "Generated home page for '$($locale.code)' is missing the Firebase chat app script."
  }
