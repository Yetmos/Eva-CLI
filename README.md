...
The repository is at the V1.7.2-alpha Lua host binding checkpoint: a compileable
...
V1.7.1 restricted Lua VM `on_event` execution, and V1.7.2 read-only Lua
context, host observability, and `ctx.tools.call` capability binding evidence,
CI gates, quickstart, release notes, and explicit known limitations.

Current managed project version: `V1.7.2-alpha` (`Cargo.toml` version
`1.7.2-alpha`, prerelease Git tag form `v1.7.2-alpha`). Version policy is defined in
...
Eva-CLI has moved past a design-only repository and now has a V1.7.2-alpha
Lua host binding runtime surface. It includes a compileable Rust workspace, configuration examples and
...
capability invocation, and CI release gates.
...
| Agent and Lua host | V1.7.2 host bindings implemented | Agent lifecycle, bounded queue, timeout/cancel/retry run control, Lua loading, sandbox gate, restricted `mlua` VM adapter, real `on_event` execution, read-only request/trace/memory tables, `ctx.host.log/audit`, `ctx.tools.call`, static parser fallback, and generation marker | Resource limits, shadow load, generation swap, and rollback remain later scope |
...
V1.7.2-alpha is a source alpha and Lua host binding evidence
...
- Lua resource limits, generation swaps, and hot reload rollback behind the Rust
  runtime boundary.
