...
hotplug safety gates, plus V1.15.7 durable memory/knowledge maintenance and
provider retrieval execution evidence.
...
schema/redaction/source-audit execution, release evidence gates, command module split coverage, and CI
release gates.
...
- Durable Scheduler, policy-driven memory redaction/audit, tracing subscriber,
  OTel/metrics exporters, and backup stores beyond the current durable
  EventBus/task snapshot/audit sink baseline, V1.15.7 provider retrieval
  executor, V1.16.1 JSONL runtime audit wiring, and local diagnostic surfaces.
