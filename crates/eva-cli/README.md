...
| `eva release check` | 聚合跨平台、稳定性、文档、安全、性能、迁移、daemon runtime、hardware safety 和 V1.17.4 public JSON contract diff readiness 门禁；可读取 V1.11 artifact、distribution、security scan 和 benchmark evidence，输出 release readiness。 |
...
| 命令 | 行为 |
| --- | --- |
| `release check` | 调用 `eva_release::ReleaseHardeningService::readiness`，聚合 release gates、platform readiness、stability scenarios 和 audit，包括 V1.15.5 `REL-HARDWARE-SAFETY-001` 与 V1.17.4 `REL-JSON-CONTRACT-001`。 |
