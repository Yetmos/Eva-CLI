...
`eva-cli` 负责命令解析、文本/JSON 输出、trace 字段和 exit code 映射。当前命令面覆盖 V1.0 core runtime、V1.1 外部能力诊断、V1.2 request-scoped memory/knowledge context、V1.3 plan-first 硬件接入诊断、V1.4 backup/lifecycle planning、V1.5 release hardening、V1.6 durable task store 入口、V1.9.3 Discovery source report、V1.9.4 durable memory context，以及 V1.9.5 observability smoke 输出。
...
| `restore apply` | P6-001 默认拒绝执行并在错误上下文写入 V1.9.2 policy decision；P6-002 在 `--dry-run` 下读取 plan 文件并验证 filesystem artifact store 中的 backup digest，仍保持 `apply_allowed:false`。 |
