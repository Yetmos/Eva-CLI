...
`eva-cli` 负责命令解析、文本/JSON 输出、trace 字段和 exit code 映射。V0.5 仍不启动后台 daemon；`run --example basic` 同步执行后，会把最新 task report 写入 `.eva/tasks`，供 `task status/logs/cancel` 跨命令读取。
...
| `eva run --example basic` | 执行 V0.5 in-memory basic event loop，并写入本地 task report。 |
...
| `--timeout-ms <ms>` | 设置 Agent handler timeout budget；`0` 会触发 V0.5 timeout 诊断路径。 |
...
V0.5 的 task 记录是本地诊断文件，不是 durable task database。目录已在 `.gitignore` 中排除。
...
已覆盖：config validate JSON、inspect text、unknown command、JSON escaping、basic run JSON、cancelled basic run、task status/logs/cancel、doctor sample project。
