...
V1.15.7 的 CLI 变化是 runtime marker 公开 `knowledge_retrieval_provider_v1.15.7`，表示 `eva-memory` 已可通过 `CapabilityHostApi` 显式 provider 入口执行受监督 retrieval、schema 校验、脱敏和 source audit。该 marker 不表示新增用户命令，也不表示 daemon smoke 会启动生产检索 provider。
...
| `eva daemon start/status/stop/shutdown/submit/cancel/drain/reload` | 验证 V1.12/V1.13.5/V1.15.4/V1.15.6 daemon pid/lock/state、durable backend、provider/task recovery、policy、observability、hardware hotplug subscriber、memory/knowledge maintenance、shutdown contract 和本机 filesystem mailbox 控制面；不启动 provider 进程，也不暴露 raw hardware handle。 |
