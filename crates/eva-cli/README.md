...
`eva-cli` 负责命令解析、文本/JSON 输出、trace 字段和 exit code 映射。当前命令面覆盖 V1.0 core runtime、V1.1 外部能力诊断、V1.2 request-scoped memory/knowledge context、V1.3 plan-first 硬件接入诊断、V1.4 backup/lifecycle planning，以及 V1.5 release hardening。

CLI 不启动后台 daemon；`run --example basic` 同步执行后，会把最新 task report 写入 `.eva/tasks`，供 `task status/logs/cancel` 跨命令读取。外部能力、记忆上下文、硬件、备份、生命周期和发布加固命令当前都以可验证诊断 surface 为主，不打开真实 stdio/http/MCP server、raw hardware I/O，也不执行 destructive restore 或真实进程升级。
