...
| `eva service install/status/start/stop/restart/uninstall` | 从项目 `service_manager` 配置调用当前 host 对应的 Windows Service、systemd 或 launchd Adapter；写操作先经过 mutation inventory。Fake 只允许显式 `--dev`，每次调用创建新的进程内 Adapter，并通过项目/服务隔离的 `.eva/service-manager` 开发状态文件恢复和持久化生命周期；它仍不属于生产 evidence。 |
...
local supervisor adapter smoke，不是生产 service manager/daemon handoff。平台 Service Adapter 与独立的 `eva service install/status/start/stop/restart/uninstall` 命令已经存在，但 `upgrade apply` 仍未接入 service-manager handoff，也没有启动 candidate daemon service entrypoint。
