...
| `eva restore apply` | 默认拒绝 destructive apply；`--dry-run` 会校验 backup artifact 与 pre-restore backup evidence。 |
...
| `restore apply` | 默认拒绝执行并在错误上下文写入 V1.9.2 policy decision；`--dry-run` 下读取 plan 文件并验证 filesystem artifact store 中的 backup digest 和 pre-restore backup evidence，仍保持 `apply_allowed:false`。 |
