...
CLI 不宣称生产后台 daemon 已完成认证；`daemon start` 验证本机 pid/lock/state、durable backend、recovery、policy、observability、hardware hotplug subscriber、memory/knowledge maintenance 和 shutdown contract。受控执行路径已包括 manifest-gated provider/MCP/Skill 调用、带确认和 policy gate 的 capability call、staged restore apply/rollback、本地 supervisor state/release-pointer handoff、host-bound 平台 service lifecycle 命令，以及 service manager 使用的隐藏 direct daemon entrypoint。该入口校验 project、host kind、service name 与 argv identity digest，在同一进程直接持有 daemon PID/lease；OS stop 只触发 cooperative token，再复用既有 drain/shutdown 事务。这一代码契约仍不等同于 raw hardware I/O、真实 host stop/boot/reboot evidence、destructive lifecycle harness、production release gate 或蓝绿进程升级。
...
| `eva service install/status/start/stop/restart/uninstall` | 从项目 `service_manager` 配置调用当前 host 对应的 Windows Service、systemd 或 launchd Adapter；生产 definition 附加规范化 executable/native argv/working directory 和 identity digest，指向隐藏 direct daemon entrypoint，写操作先经过 mutation inventory。Fake 只允许显式 `--dev`，每次调用创建新的进程内 Adapter，并通过项目/服务隔离的 `.eva/service-manager` 开发状态文件恢复和持久化生命周期；Fake 与本地 adapter 测试都不属于真实 host evidence。 |
...
local supervisor adapter smoke，不是生产 service manager/daemon handoff。平台 Service Adapter、独立的 `eva service install/status/start/stop/restart/uninstall` 命令和 active daemon 的 hidden direct entrypoint 已经存在，但 `upgrade apply` 仍未接入 service-manager candidate handoff，不会启动第二个 candidate Runtime，也没有实现真实 blue-green/ingress 切换。
