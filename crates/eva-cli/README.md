...
V1.11.4 已把 release 命令组的 parser、evidence reader、writer 和 JSON formatter 拆入 `src/run/release_cmd.rs`；`run.rs` 保留顶层 dispatch、共享 formatter helper、trace 和 exit code 映射。拆分后的 release 回归继续覆盖 `release_check`、`release_perf` 和 V1.5 release hardening JSON contract。
