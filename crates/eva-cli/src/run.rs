...
const RELEASE_LABEL: &str = "V1.6.3-alpha";
const RELEASE_RUNTIME_MODE: &str =
    "in_memory_v1.0 + external_capability_v1.1 + context_v1.2 + hardware_v1.3 + lifecycle_v1.4 + release_v1.5 + durable_backend_v1.6.1 + durable_eventbus_v1.6.2 + durable_task_audit_artifact_v1.6.3";
...
fn parse_release_migration_options(args: &[String]) -> Result<ReleaseMigrationOptions, EvaError> {
    ...
    let mut to_version = "1.6.3-alpha".to_owned();
    let mut index = 0;
    while index < args.len() {
        match args[index].as_str() {
            "--from" | "--from-version" => {
...
mod tests {
    ...
    #[test]
    fn version_text_and_json_report_v163_durable_task_audit_artifact_alpha() {
        let (text_exit, text_stdout, text_stderr) = run_cli(&["--version"]);
        assert_eq!(text_exit, EXIT_OK, "{text_stderr}");
        assert!(text_stdout.contains("eva 1.6.3-alpha"));
        assert!(text_stdout.contains("V1.6.3-alpha"));
        ...
        assert!(json_stdout.contains("\"version\":\"1.6.3-alpha\""));
        assert!(json_stdout.contains("\"release\":\"V1.6.3-alpha\""));
    ...
    fn v15_release_hardening_commands_report_json() {
        ...
        assert!(migration_stdout.contains("\"to_version\":\"1.6.3-alpha\""));
