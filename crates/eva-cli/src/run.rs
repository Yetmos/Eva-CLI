...
fn parse_release_migration_options(args: &[String]) -> Result<ReleaseMigrationOptions, EvaError> {
    ...
    let mut to_version = "1.7.3-alpha".to_owned();
    let mut index = 0;
    while index < args.len() {
        match args[index].as_str() {
            "--from" | "--from-version" => {
...
mod tests {
    ...
    #[test]
    fn version_text_and_json_report_v173_lua_resource_limits_alpha() {
        let (text_exit, text_stdout, text_stderr) = run_cli(&["--version"]);
        assert_eq!(text_exit, EXIT_OK, "{text_stderr}");
        assert!(text_stdout.contains("eva 1.7.3-alpha"));
        assert!(text_stdout.contains("V1.7.3-alpha"));
        ...
        assert!(json_stdout.contains("\"version\":\"1.7.3-alpha\""));
        assert!(json_stdout.contains("\"release\":\"V1.7.3-alpha\""));
    ...
    fn v15_release_hardening_commands_report_json() {
        ...
        assert!(migration_stdout.contains("\"to_version\":\"1.7.3-alpha\""));
