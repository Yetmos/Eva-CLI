...
| `run/observability_cmd.rs` | V1.16.2 已更新 | `observability smoke` parser、file JSONL backend smoke、tracing subscriber bridge smoke、text/JSON writer 和 degraded report formatter；保持 audit/metrics/span 计数、continuity key 和追加的 `tracing_bridge` 输出稳定。 |
...
| `run/version_cmd.rs` runtime marker | V1.16.1 已更新 | `version` / `--version` 输出新增 `mcp_http_auth_v1.13.6`、`mcp_compat_matrix_v1.13.7`、`provider_supervision_release_gate_v1.13.8`、`restore_staged_mutation_planner_v1.14.1`、`restore_file_mutation_engine_v1.14.2`、`restore_rollback_apply_v1.14.3`、`restore_operator_confirmation_v1.14.4`、`service_manager_abstraction_v1.14.5`、`hardware_os_permission_provider_v1.15.1`、`hardware_hotplug_subscriber_v1.15.4`、`hardware_safety_release_gate_v1.15.5`、`memory_knowledge_maintenance_v1.15.6`、`knowledge_retrieval_provider_v1.15.7`、`memory_redaction_audit_v1.15.8` 和 `runtime_audit_sink_wiring_v1.16.1` 和 `tracing_subscriber_bridge_v1.16.2`，用于标识 tracing bridge、MCP HTTP/auth transport boundary、compatibility matrix release gate、provider supervision release gate、restore staged mutation planner、restore file mutation engine、restore rollback apply、operator confirmation 输出、service-manager 抽象层 readiness、硬件 OS permission provider 诊断、daemon hotplug subscriber、alpha simulator-only hardware safety gate、durable memory/knowledge maintenance baseline、受监督 knowledge retrieval provider execution baseline、memory redaction/audit baseline 和 runtime/provider/task/restore observability wiring baseline。 |
...
- V1.16.2 adds the `tracing_subscriber_bridge_v1.16.2` runtime marker and maps
  tracing span/event smoke records into existing JSONL/dev-console sinks; no
...
cargo run -- observability smoke --backend .eva/ci-observability --output json`ncargo run -- observability smoke --backend .eva/ci-observability --tracing-sink dev-console --output json
