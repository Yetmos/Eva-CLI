...
| `run.rs` | V1.17.3 已更新 | 顶层命令路由、共享 JSON envelope/trace helper、text risk helper、formatter helper、exit code 映射和 runtime marker；`run` 仅保留到 `run/run_cmd.rs` 的路由。 |
...
| `run/release_cmd.rs` | V1.15.5 已更新 | `release check/security/perf/migration` 的 parser、artifact/distribution/security scan/benchmark evidence reader、文本/JSON writer 和 release report formatter；`release check` 现在输出 `REL-HARDWARE-SAFETY-001`，并保持 V1.11.1-V1.11.3 release evidence gate 的公开 JSON shape 与 exit code 不变。 |
...
- `release check`：调用 `eva_release::ReleaseHardeningService::readiness`，输出跨平台、稳定性、文档、安全、性能、迁移、V1.6.4 durable recovery、V1.6.5 durable diagnostics、V1.12.6 daemon runtime readiness 和 V1.15.5 hardware safety readiness 门禁。
