...
| `run/version_cmd.rs` runtime marker | V1.16.1 已更新 | `version` / `--version` 输出新增 `mcp_http_auth_v1.13.6`、`mcp_compat_matrix_v1.13.7`、`provider_supervision_release_gate_v1.13.8`、`restore_staged_mutation_planner_v1.14.1`、`restore_file_mutation_engine_v1.14.2`、`restore_rollback_apply_v1.14.3`、`restore_operator_confirmation_v1.14.4`、`service_manager_abstraction_v1.14.5`、`hardware_os_permission_provider_v1.15.1`、`hardware_hotplug_subscriber_v1.15.4`、`hardware_safety_release_gate_v1.15.5`、`memory_knowledge_maintenance_v1.15.6`、`knowledge_retrieval_provider_v1.15.7`、`memory_redaction_audit_v1.15.8` 和 `runtime_audit_sink_wiring_v1.16.1`，用于标识 MCP HTTP/auth transport boundary、compatibility matrix release gate、provider supervision release gate、restore staged mutation planner、restore file mutation engine、restore rollback apply、operator confirmation 输出、service-manager 抽象层 readiness、硬件 OS permission provider 诊断、daemon hotplug subscriber、alpha simulator-only hardware safety gate、durable memory/knowledge maintenance baseline、受监督 knowledge retrieval provider execution baseline、memory redaction/audit baseline 和 runtime/provider/task/restore observability wiring baseline。 |
...
- V1.16.1 adds the `runtime_audit_sink_wiring_v1.16.1` runtime marker and
  ...
  best-effort pipeline; no tracing subscriber or OTel exporter is installed.
...
V1.16.1 后，daemon control/task lifecycle/scheduler retry、provider supervision 和 restore apply/rollback 复用同一 `BestEffortObservabilityPipeline` 写 JSONL evidence。该 wiring 只使用现有 file backend，不安装 tracing subscriber 或真实 OpenTelemetry exporter。
