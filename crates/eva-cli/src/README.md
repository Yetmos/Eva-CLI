...
| `run/version_cmd.rs` runtime marker | V1.13.8 已更新 | `version` / `--version` 输出新增 `mcp_http_auth_v1.13.6`、`mcp_compat_matrix_v1.13.7` 和 `provider_supervision_release_gate_v1.13.8`，用于标识 MCP HTTP/auth transport boundary、compatibility matrix release gate 和 provider supervision release gate。 |
