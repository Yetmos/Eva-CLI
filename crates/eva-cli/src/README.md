...
| `run/restore_cmd.rs` | V1.14.2 已更新 | `restore plan/apply` parser、restore apply dry-run、filesystem artifact/lock gate、policy/health/rollback writer、staged mutation plan parser/formatter、mutation engine 调用和 restore JSON formatter；旧 no-step plan 保持 gated `mutation_executed:false` contract，带 staged steps 的 apply 可输出 `mutation_executed:true`。 |
...
| `run/version_cmd.rs` runtime marker | V1.14.2 已更新 | `version` / `--version` 输出新增 `mcp_http_auth_v1.13.6`、`mcp_compat_matrix_v1.13.7`、`provider_supervision_release_gate_v1.13.8`、`restore_staged_mutation_planner_v1.14.1` 和 `restore_file_mutation_engine_v1.14.2`，用于标识 MCP HTTP/auth transport boundary、compatibility matrix release gate、provider supervision release gate、restore staged mutation planner 和 restore file mutation engine。 |
