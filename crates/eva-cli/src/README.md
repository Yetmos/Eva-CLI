...
本目录承载 CLI 命令解析、执行分发、文本/JSON 输出、exit code 映射和本地/持久诊断文件读写。V1.11.4 开始把稳定命令组从 `run.rs` 拆到子模块：`version`、`doctor`、`config validate`、`inspect`、`task`、`adapter`、`mcp`、`skill`、`discovery`、`memory`、`observability`、`hardware`、`backup`、`snapshot`、`restore`、`upgrade` 和 `release` 已迁移到 `run/` 子模块；V1.17.1 继续把 `run --example basic` 的 parser、writer 和 task-store glue 拆到 `run/run_cmd.rs`，并复用 `run.rs` 的 JSON envelope、trace 字段和 exit code helper；V1.17.2 固定 capability/restore/upgrade/hardware 的 operator execution-state 字段边界；V1.17.3 固定 restore/upgrade/hardware 高风险 text summary；V1.17.4 固定 public JSON contract diff suite；V1.17.6 固定 release closure gate/report JSON 边界。
...
| `run.rs` | V1.17.6 已更新 | 顶层命令路由、共享 JSON envelope/trace helper、text risk helper、formatter helper、exit code 映射和 runtime marker；`run` 仅保留到 `run/run_cmd.rs` 的路由，version 输出新增 `v1x_closure_gate_v1.17.6`。 |
| `run/run_cmd.rs` | V1.17.1 已实现 | `run --example basic` parser、in-memory basic runtime glue、task snapshot 写入、text/JSON writer 和 run report formatter；保持 public JSON/text 输出不变。 |
...
| `run/memory_cmd.rs` | V1.15.8 已更新 | `memory context` parser、in-memory/durable context seed、policy-driven redaction、memory read/search/context JSONL observability、text/JSON writer、memory/knowledge/Lua context formatter；保持 expiration 和 durable round trip 输出不变。 |
| `run/observability_cmd.rs` | V1.16.3 已更新 | `observability smoke` parser、file JSONL backend smoke、tracing subscriber bridge smoke、OpenTelemetry SDK exporter smoke、text/JSON writer 和 degraded report formatter；保持 audit/metrics/span 计数、continuity key、`tracing_bridge` 和 `otel_exporter` 输出稳定。V1.16.4 retention/rotation policy 不隐式接入 CLI 删除动作，只通过 runtime marker 暴露能力基线。 |
| `run/hardware_cmd.rs` | V1.17.3 已更新 | `hardware list/probe/bind` parser、project device discovery、bind plan、runtime policy gate audit 和 hardware candidate/plan formatter；`bind` 输出顶层 `mutation_executed:false`，text 输出新增 operator summary、target、final state、rollback path 和 risk 摘要，并保持 plan-first、不打开 raw I/O 和 JSON envelope 不变。 |
...
| `run/restore_cmd.rs` | V1.17.3 已更新 | `restore plan/apply/rollback` parser、restore apply dry-run、filesystem artifact/lock gate、policy/health/rollback writer、staged mutation plan parser/formatter、mutation/rollback engine 调用、operator confirmation 输出、restore JSON formatter 和非 dry-run apply/rollback observability JSONL 写入；plan/dry-run/rollback 均输出顶层 `mutation_executed`，text 输出新增 `final_state`、`rollback_path` 和 risk 摘要，旧 no-step plan 保持 gated `mutation_executed:false` contract，带 staged steps 的 apply 可输出 `mutation_executed:true`，rollback 继续保留 `rollback_executed:true`。 |
| `run/upgrade_cmd.rs` | V1.17.3 已更新 | `upgrade check/apply` parser、apply lock、state-store supervisor handoff、runtime binary smoke、release pointer mutation 和 rollback formatter；`check` 输出 `mutation_executed:false`，`apply` 输出顶层 `mutation_executed` 并与 handoff mutation 状态保持一致，text 输出新增 operator summary、target generation、final state、rollback path 和 risk 摘要，lock-only 与 handoff JSON contract 继续兼容。 |
| `run/release_cmd.rs` | V1.17.6 已更新 | `release check/security/perf/migration` 的 parser、artifact/distribution/security scan/benchmark evidence reader、文本/JSON writer 和 release report formatter；`release check` 现在输出 `REL-HARDWARE-SAFETY-001`、`REL-OBSERVABILITY-POLICY-001`、`REL-JSON-CONTRACT-001`、`REL-V1X-CLOSURE-001` 和 additive `closure` JSON，并保持 V1.11.1-V1.11.3 release evidence gate 的公开 JSON shape 与 exit code 不变。 |
...
| `run/daemon_cmd.rs` | V1.16.1 已更新 | `daemon start/status/stop/shutdown/submit/cancel/drain/reload` parser、foreground daemon smoke/control mailbox writer；`start` JSON 新增 task/provider process recovery report、`hardware_hotplug` subscriber evidence 和 `memory_maintenance` TTL GC/rebuild checkpoint evidence；control/submit/cancel/scheduler retry 路径写入 best-effort observability JSONL。 |
| `run/version_cmd.rs` runtime marker | V1.17.6 已更新 | `version` / `--version` 输出新增 `mcp_http_auth_v1.13.6`、`mcp_compat_matrix_v1.13.7`、`provider_supervision_release_gate_v1.13.8`、`restore_staged_mutation_planner_v1.14.1`、`restore_file_mutation_engine_v1.14.2`、`restore_rollback_apply_v1.14.3`、`restore_operator_confirmation_v1.14.4`、`service_manager_abstraction_v1.14.5`、`hardware_os_permission_provider_v1.15.1`、`hardware_hotplug_subscriber_v1.15.4`、`hardware_safety_release_gate_v1.15.5`、`memory_knowledge_maintenance_v1.15.6`、`knowledge_retrieval_provider_v1.15.7`、`memory_redaction_audit_v1.15.8`、`runtime_audit_sink_wiring_v1.16.1`、`tracing_subscriber_bridge_v1.16.2`、`opentelemetry_sdk_exporter_v1.16.3`、`observability_retention_policy_v1.16.4`、`run_command_module_split_v1.17.1`、`operator_execution_fields_v1.17.2`、`operator_apply_text_v1.17.3`、`json_contract_diff_suite_v1.17.4` 和 `v1x_closure_gate_v1.17.6`，用于标识 V1.x closure release gate、public JSON contract diff suite、high-risk apply text summary、operator execution-state 字段、run command module split、observability retention/rotation policy、OpenTelemetry SDK exporter smoke、tracing bridge、MCP HTTP/auth transport boundary、compatibility matrix release gate、provider supervision release gate、restore staged mutation planner、restore file mutation engine、restore rollback apply、operator confirmation 输出、service-manager 抽象层 readiness、硬件 OS permission provider 诊断、daemon hotplug subscriber、alpha simulator-only hardware safety gate、durable memory/knowledge maintenance baseline、受监督 knowledge retrieval provider execution baseline、memory redaction/audit baseline 和 runtime/provider/task/restore observability wiring baseline。 |
| `run/capability_cmd.rs` | V1.17.2 已确认 | `capability list/probe/call` parser、CapabilityRegistry/provider selection/permission gate/runtime policy/adapter-backed host evidence 构造、text/JSON formatter；`call` 默认 dry-run，确认后受控 invoke，并同时输出 `invocation_executed` 与 `mutation_executed:false`。 |
...
- V1.15.6 lets `daemon start` run one durable memory/knowledge maintenance
  smoke with `index.lock`, `memory-gc.checkpoint`,
  `knowledge-rebuild.checkpoint`, and `memory.maintenance` audit evidence.
- V1.15.7 adds the `knowledge_retrieval_provider_v1.15.7` runtime marker for
  the memory-layer supervised retrieval provider executor; no CLI command starts
  a production retrieval provider implicitly.
- V1.15.8 adds the `memory_redaction_audit_v1.15.8` runtime marker and lets
  `memory context` write memory read/search/context audit and metrics JSONL while
  using `memory_policy.redaction`.
- V1.16.1 adds the `runtime_audit_sink_wiring_v1.16.1` runtime marker and
  writes daemon/provider/task/restore observations to the existing JSONL
  best-effort pipeline.
- V1.16.2 adds the `tracing_subscriber_bridge_v1.16.2` runtime marker and maps
  tracing span/event smoke records into existing JSONL/dev-console sinks.
- V1.16.3 adds the `opentelemetry_sdk_exporter_v1.16.3` runtime marker and lets
  `observability smoke --otel-endpoint <url>` explicitly run SDK OTLP
  HTTP/protobuf trace/metrics exporter smoke.
- V1.16.4 adds the `observability_retention_policy_v1.16.4` runtime marker for
  JSONL/durable-audit retention, rotation, max-size, and corrupt-record policy;
  real database sink remains outside this slice.
- V1.17.1 adds the `run_command_module_split_v1.17.1` runtime marker after
  moving `run --example basic` parser/runtime glue/task snapshot/output code to
  `run/run_cmd.rs`.
- V1.17.2 adds the `operator_execution_fields_v1.17.2` runtime marker and fixes
  the operator-facing distinction between invocation execution and destructive
  mutation execution.
- V1.17.3 adds the `operator_apply_text_v1.17.3` runtime marker and text-only
  summaries for restore/upgrade/hardware high-risk paths.
- V1.17.4 adds the `json_contract_diff_suite_v1.17.4` runtime marker plus
  golden subset JSON fixtures under `contracts/cli-json/`; the validation
  script blocks removed or renamed fields while allowing additive fields.
- V1.17.6 adds the `v1x_closure_gate_v1.17.6` runtime marker and makes
  `release check` emit `REL-OBSERVABILITY-POLICY-001`,
  `REL-V1X-CLOSURE-001`, and additive `closure` JSON with production-only
  external blockers.
...
- `hardware bind --adapter <id>`：生成 `HardwareBindPlan`，包含 candidate、status、apply 标记、OS permission evidence、plan steps、risks、V1.9.2 policy audit 和 `mutation_executed:false`。
...
- `restore plan`：调用 `ReleaseSnapshotService::restore_plan`，输出 `apply_allowed:false` 与 `mutation_executed:false`，并可通过 `--artifact-store <path>` 生成 filesystem backup evidence。
- `restore apply`：带 `--dry-run` 时读取 plan 文件并验证 filesystem artifact store 中的 backup artifact key/digest 与 pre-restore backup evidence，保持 `apply_allowed:false`，并在 plan 声明 staged mutation steps 时输出 plan-only `mutation_plan`；非 dry-run 必须带 `--lock-store`，默认 project policy 拒绝 `restore.apply` 且不会留下锁文件，显式 allow 后获取 `{plan_id}.restore.lock` 并运行 health gate。无 mutation steps 时返回 `status:"gated"`、`mutation_executed:false`；有 staged steps 时调用 `RestoreMutationEngine` 写目标文件和 `{plan_id}.restore.txn`，成功返回 `status:"applied"`、`mutation_executed:true`，失败返回 `rollback_required`；V1.16.1 非 dry-run 成功路径写 `.eva/data/observability` audit/metric/span JSONL。
- V1.17.2：`restore apply --dry-run` 显式输出 `mutation_executed:false`；`restore rollback` 输出顶层 `mutation_executed` 并继续保留 `rollback_executed`，避免把 rollback apply 与普通 dry-run/gated 输出混读；V1.17.3：restore text 输出补 `final_state`、`rollback_path` 和 risk 摘要。
- `upgrade check`：调用 `eva_lifecycle` 的 in-memory supervisor、generation、drain、rollback 状态机，并结合 migration preflight，输出 `mutation_executed:false`。
- `upgrade apply`：默认仍是 lock-only 且 `mutation_executed:false`；带 `--state-store` 时还要求 `supervisor.handoff` 和 `release.pointer_mutation` policy allow，通过 runtime binary smoke 和 health gate 后写 `state/release-pointer` 与 handoff state，顶层 `mutation_executed` 与 handoff mutation 状态保持一致，health 失败输出 rollback plan 且不写 pointer；V1.17.3 text 输出补 operator summary、target generation、final state、rollback path 和 risk 摘要。

这些命令是 release/lifecycle readiness smoke。V1.10.4 `restore apply` 已补 confirmation、artifact evidence、policy approval、apply lock、health check 和 rollback-required 输出；V1.14.1 追加 copy/delete/replace staged mutation preview、affected paths、preflight hash 和 rollback manifest；V1.14.2 已在 staged plan 存在且 gate 全部通过时执行文件 mutation 并写 transaction log；V1.14.3 `restore rollback` 可在 confirmation、artifact evidence、policy、rollback lock、health、transaction log 和 pre-restore archive entry 全部通过后执行反向文件恢复；V1.14.4 在 dry-run/apply/rollback text 与 JSON 中追加 `operator_confirmation`，显式列出 confirm token、target root、affected count、状态位和不可逆风险；V1.16.1 对非 dry-run apply/rollback 成功路径写 JSONL observability evidence。V1.10.5 `upgrade apply --state-store` 会执行本地 release pointer mutation 和 handoff state 持久化，但仍不是生产 service-manager/daemon handoff；V1.14.5 只新增 service-manager trait、typed config、fake adapter evidence 和版本 marker。
...
- `release check`：调用 `eva_release::ReleaseHardeningService::readiness`，输出跨平台、稳定性、文档、安全、性能、迁移、V1.6.4 durable recovery、V1.6.5 durable diagnostics、V1.12.6 daemon runtime readiness、V1.15.5 hardware safety readiness、V1.16 observability policy readiness、V1.17.4 public JSON contract diff 和 V1.17.6 V1.x closure 门禁。
...
V1.16.1 后，daemon control/task lifecycle/scheduler retry、provider supervision 和 restore apply/rollback 复用同一 `BestEffortObservabilityPipeline` 写 JSONL evidence。V1.16.2 后，`observability smoke` 还会通过 tracing subscriber bridge 把 span/event 写入 JSONL 或 dev-console sink，并验证脱敏。V1.16.3 后，传入 `--otel-endpoint <url>` 会显式运行 OpenTelemetry SDK OTLP HTTP/protobuf trace/metrics exporter smoke；collector 不可用时 `otel_exporter.degraded:true`，命令仍成功返回。未传 endpoint 时 `otel_exporter:null`，不会隐式访问外部 collector。V1.16.4 后，retention/rotation policy 由 `eva-observability`/`eva-storage` API 和配置 schema 暴露，CLI 不隐式删除观测文件。
...
`version`、`doctor`、`config validate`、`inspect`、`run --example basic`、`task`、`adapter`、`mcp`、`skill`、`discovery`、`memory`、`observability`、`hardware`、`backup`、`snapshot`、`restore`、`upgrade` 和 `release` 已完成拆分；后续新增命令模块时，仍不能改变公开 JSON envelope。
