...
| `run/restore_cmd.rs` | V1.14.1 已更新 | `restore plan/apply` parser、restore apply dry-run、filesystem artifact/lock gate、policy/health/rollback writer、staged mutation plan parser/formatter 和 restore JSON formatter；保持 staged `mutation_executed:false` contract 不变。 |
...
| `run/version_cmd.rs` runtime marker | V1.14.1 已更新 | `version` / `--version` 输出新增 `mcp_http_auth_v1.13.6`、`mcp_compat_matrix_v1.13.7`、`provider_supervision_release_gate_v1.13.8` 和 `restore_staged_mutation_planner_v1.14.1`，用于标识 MCP HTTP/auth transport boundary、compatibility matrix release gate、provider supervision release gate 和 restore staged mutation planner。 |
