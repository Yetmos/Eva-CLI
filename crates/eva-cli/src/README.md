...
本目录承载 CLI 命令解析、执行分发、文本/JSON 输出、exit code 映射和本地/持久诊断文件读写。V1.11.4 开始把稳定命令组从 `run.rs` 拆到子模块：`version`、`doctor`、`config validate`、`inspect`、`task`、`adapter`、`mcp`、`skill`、`discovery`、`memory`、`observability`、`hardware`、`backup`、`snapshot`、`restore`、`upgrade` 和 `release` 已迁移到 `run/` 子模块；V1.17.1 继续把 `run --example basic` 的 parser、writer 和 task-store glue 拆到 `run/run_cmd.rs`，并复用 `run.rs` 的 JSON envelope、trace 字段和 exit code helper。
...
| `run.rs` | V1.17.1 已收敛 | 顶层命令路由、共享 JSON envelope/trace helper、formatter helper 和 exit code 映射；`run` 仅保留到 `run/run_cmd.rs` 的路由。 |
...
| `run/restore_cmd.rs` | V1.16.1 已更新 | `restore plan/apply/rollback` parser、restore apply dry-run、filesystem artifact/lock gate、policy/health/rollback writer、staged mutation plan parser/formatter、mutation/rollback engine 调用、operator confirmation 输出、restore JSON formatter 和非 dry-run apply/rollback observability JSONL 写入；旧 no-step plan 保持 gated `mutation_executed:false` contract，带 staged steps 的 apply 可输出 `mutation_executed:true`，rollback 可输出 `rollback_executed:true`。 |
