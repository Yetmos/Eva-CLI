...
| `run.rs` | V1.17.2 已更新 | 顶层命令路由、共享 JSON envelope/trace helper、formatter helper、exit code 映射和 runtime marker；`run` 仅保留到 `run/run_cmd.rs` 的路由。 |
...
| `run/hardware_cmd.rs` | V1.17.2 已更新 | `hardware list/probe/bind` parser、project device discovery、bind plan、runtime policy gate audit 和 hardware candidate/plan formatter；`bind` 输出顶层 `mutation_executed:false`，并保持 plan-first、不打开 raw I/O 和 JSON envelope 不变。 |
...
| `run/restore_cmd.rs` | V1.17.2 已更新 | `restore plan/apply/rollback` parser、restore apply dry-run、filesystem artifact/lock gate、policy/health/rollback writer、staged mutation plan parser/formatter、mutation/rollback engine 调用、operator confirmation 输出、restore JSON formatter 和非 dry-run apply/rollback observability JSONL 写入；plan/dry-run/rollback 均输出顶层 `mutation_executed`，旧 no-step plan 保持 gated `mutation_executed:false` contract，带 staged steps 的 apply 可输出 `mutation_executed:true`，rollback 继续保留 `rollback_executed:true`。 |
| `run/upgrade_cmd.rs` | V1.17.2 已更新 | `upgrade check/apply` parser、apply lock、state-store supervisor handoff、runtime binary smoke、release pointer mutation 和 rollback formatter；`check` 输出 `mutation_executed:false`，`apply` 输出顶层 `mutation_executed` 并与 handoff mutation 状态保持一致，lock-only 与 handoff JSON contract 继续兼容。 |
...
- V1.17.2：`restore apply --dry-run` 显式输出 `mutation_executed:false`；`restore rollback` 输出顶层 `mutation_executed` 并继续保留 `rollback_executed`，避免把 rollback apply 与普通 dry-run/gated 输出混读。
...
- `upgrade apply`：默认仍是 lock-only 且 `mutation_executed:false`；带 `--state-store` 时还要求 `supervisor.handoff` 和 `release.pointer_mutation` policy allow，通过 runtime binary smoke 和 health gate 后写 `state/release-pointer` 与 handoff state，顶层 `mutation_executed` 与 handoff mutation 状态保持一致，health 失败输出 rollback plan 且不写 pointer。
