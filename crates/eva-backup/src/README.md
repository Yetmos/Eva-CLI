...
本目录承载 V1.4/V1.10/V1.14 备份、迁移包、release snapshot、signed archive、pre-restore evidence、restore apply gate、staged mutation planner 和 manifest 完整性校验源码。实现重点是把高风险恢复路径表达成可测试的 plan、archive、manifest、digest、signature、lock、health、preview、rollback manifest 和 audit，而不是直接执行 destructive restore。
...
| `restore_apply.rs` | restore apply validation, staged mutation planning, lock, policy and health gate | 已完成 V1.14.1 | `RestoreApplyPlan`、`PreRestoreBackupEvidence`、`RestoreMutationStep`、`RestoreStagedMutationPlanner`、`RestoreApplyValidator`、`RestoreApplyCoordinator`、`RestoreApplyLockStore`、`RestoreApplyHealthCheck`，验证 durable artifact 和 pre-restore evidence，生成 plan-only mutation preview/preflight hash/rollback manifest，获取 apply lock 并输出 gated/blocked report，不执行恢复。 |
...
- V1.14.1 gated report 保持 `mutation_executed:false`，不能移动 release pointer、启动 supervisor handoff 或修改 workspace。
