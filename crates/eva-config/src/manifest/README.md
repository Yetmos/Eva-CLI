...
本目录承载 Agent、Adapter、Capability 三类 manifest 的配置结构和基础校验。V0.2 已完成最小字段解析，V1.1 会继续扩展 Adapter/MCP/Discovery 相关字段。
...
| `adapter.rs` | Adapter ID、transport、capability provider 声明 | 已完成 | V0.2/V1.1 |
...
| 4 | 后续扩展 transport、schema、policy 字段。 | V1.x 外部能力可配置。 |
...
| Adapter manifest | Adapter 注册前配置 | 已完成 | 扩展 transport schema。 |
