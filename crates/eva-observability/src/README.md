...
本目录承载 trace、audit、metrics 契约、V1.9.5 file JSONL backend 基线、V1.13.2 provider credential session audit action、V1.15.6 memory maintenance audit action、V1.15.8 memory write/read/search/context audit actions、V1.16.1 runtime/provider/task/restore audit sink wiring 动作、V1.16.2 tracing subscriber bridge、V1.16.3 OpenTelemetry SDK exporter smoke 和 V1.16.4 retention/rotation policy。后续模块只应追加稳定动作、指标或后端 adapter，不应改变已公开字段含义。
...
| `backend.rs` | `FileObservabilitySink`、`BestEffortObservabilityPipeline`、OTel-style span JSONL export、smoke report、JSONL rotation/retention/corrupt report | 已完成 V1.16.4 | V1.9.5/V1.16.4 |
| `retention.rs` | `ObservabilityRetentionPolicy`、sink policy kind、corrupt-record policy 和 retention report | 已完成 V1.16.4 | V1.16.4 |
...
| 7 | V1.16.4 追加 retention/rotation、max-size、corrupt-record policy 和 database policy kind，不更改已有字段。 | JSONL/durable-audit 保留策略可测试；真实 database sink 后续接入。 |
...
| Backend | file JSONL、OTel-style span export、best-effort degradation、JSONL rotation/retention/corrupt report | 已完成 V1.16.4 | 已接 runtime/provider/task/restore JSONL wiring；后续接真实 db sink。 |
| Retention policy | JSONL/durable-audit/database policy kind、max size、retain window、corrupt-record handling | 已完成 V1.16.4 | database kind 仍是策略边界，不代表真实 DB backend。 |
| Tracing bridge | tracing subscriber span/event bridge | 已完成 V1.16.2 | 后续 retention/db policy 复用该映射。 |
| OpenTelemetry exporter | SDK OTLP HTTP/protobuf trace/metrics exporter smoke | 已完成 V1.16.3 | 后续接常驻 runtime exporter 配置和真实 db sink。 |
