...
| `audit.rs` | `AuditAction`、`AuditOutcome`、`AuditEvent`、`AuditSink` | V1.8.3 已更新 | V0.2/V1.x |
...
| 2 | 定义 audit action/outcome 和 sink trait。 | 高风险操作可审计；V1.6.4 追加 `runtime.recovered`，V1.8.3 追加 MCP session/stream/proxy actions。 |
...
| Audit | 审计事件和 sink | 已完成 V1.8.3 | 增加后续高风险 apply 和分发动作。 |
