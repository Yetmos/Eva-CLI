...
| 6 | 后续按模块追加 action、metric 和生产 exporter，不更改已有字段。 | 兼容 CLI 和后端。 |
...
| Backend | file JSONL、OTel-style span export、best-effort degradation | 已完成 V1.16.1 | 已接 runtime/provider/task/restore JSONL wiring；后续接 OpenTelemetry SDK exporter、db sink、retention/rotation。 |
| Tracing bridge | tracing subscriber span/event bridge | 已完成 V1.16.2 | 后续 OTel exporter/retention 复用该映射。 |
