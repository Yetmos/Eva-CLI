...
| `invoke` | 已完成 | `InvokeTarget`、`InvokeRequest`、`InvokeResponse`、`InvokeStatus`、`InvokeMetadata`，含 accepted/completed/failed/cancelled/timeout 构造器；`timeout_with_error` 可保留 provider code 和 safe context。 | 不执行真实 Agent/Adapter 调用；执行层属于 runtime/adapter crate。 |
...
| 构造带错误上下文的超时响应 | `InvokeResponse::timeout_with_error` | status 为 `Timeout`，保留调用层已构造的 `EvaError`。 |
...
| `timeout_response_can_preserve_structured_error_context` | Timeout response 可保留 provider code 和 safe context。 |
