...
`eva-storage` 负责 Eva-CLI 的状态、事件日志和 artifact 存储契约。V0.4 已实现标准库 in-memory 版本，用于 `examples/basic` 和单元测试；SQLite/local durable backend 仍保留为后续实现边界。
...
主要 re-export 位于 `src/lib.rs`，下游 crate 不需要直接引用子模块路径。
...
V0.4 已覆盖：事件 append/watermark、ack consumer、fail structured error、replay cursor、StateStore 版本冲突、ArtifactStore digest round trip。
...
| V1.4 | 实现 migration、snapshot、release artifact 的 durable backend。 |
