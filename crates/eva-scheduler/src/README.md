...
| `retry.rs` | V1.13.3 已实现 | `dispatch_retry_event` 将 durable redrive replay event 投递到 scheduler mailbox，并返回 delivery evidence；`RetryBackoffPolicy` / `decide_retry_backoff` 为 retryable provider admission failure 生成 backoff 入队判定。 |
