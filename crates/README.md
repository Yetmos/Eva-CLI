...
## V1.4 Workspace Status

V1.4 keeps the V1.1 external capability checkpoint, V1.2 request-scoped memory/knowledge context, and V1.3 controlled hardware boundary, then adds backup and lifecycle planning across `eva-backup`, `eva-lifecycle`, and `eva-cli`:
...
- `eva-cli`: `adapter list/probe`, `mcp list/probe`, `skill list/run`, `discovery scan`, `memory context`, `hardware list/probe/bind`, `backup create`, `snapshot create`, `restore plan`, and `upgrade check` commands with text/JSON envelopes.

V1.4 proves that backup, restore, and upgrade lifecycle operations can be planned and audited without destructive mutation or real process management.
...
| `eva-cli` | CLI parser、formatter、exit code、运行入口、version/task/external/memory/hardware/backup/lifecycle 命令 | 已完成 V1.4 backup/lifecycle command surface | [README](eva-cli/README.md) |
