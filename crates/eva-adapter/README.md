...
V1.1 已实现外部能力的受控 envelope；V1.3 在此基础上实现 hardware transport，使硬件调用必须经由 `eva-hardware` 的 registry、lease 和 driver binding，不允许 Lua 直接访问 raw I/O。V1.8.1 将 stdio/http runner 接入 `AdapterRuntime`，外部 provider 可通过 manifest command/endpoint/env/limits 进入受控执行路径。
...
| Runtime | 已完成 V1.8.1 | `AdapterRuntime` 提供 list/probe/invoke；probe 无副作用；invoke 可执行 builtin/MCP/Skill/hardware envelope 与受控 stdio/http runner。 |
...
| `src/runtime.rs` | transport 执行 | 已完成 V1.8.1 | 后续补 provider supervision、并发限制、熔断和更丰富 provider-specific safe context。 |
...
`eva-adapter` owns Adapter runtime descriptors, registry, routing, controlled transport execution, and provider error mapping. V1.8.1 wires stdio/http runners into `AdapterRuntime` with manifest-derived command/endpoint/env/limits and credential redaction; broader provider supervision remains future work.
