...
这些实现证明 AdapterRuntime 的调用 envelope、trace、audit 和错误映射可以用 fake provider 和受控 runner 测试。V1.8.2 后 stdio/http 和 MCP JSON-RPC tool call 已能通过 AdapterRuntime 执行，但 provider supervision、并发/限流、streaming 和更完整认证会在后续节点补齐。
...
| MCP server 生命周期失控 | MCP invoke 当前只做单次 stdio JSON-RPC tool call 并在结束后停止进程；长生命周期 session registry、streaming 和 orphan cleanup 留到 V1.8.3。 |
