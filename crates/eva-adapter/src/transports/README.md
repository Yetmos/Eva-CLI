...
| `mcp.rs` | MCP tool/resource/prompt | 已完成 P5 | 使用 `eva-mcp::McpAllowlist` 和 in-memory client surface，并读取 MCP session config；当前不启动真实 server。 |
...
这些实现证明 AdapterRuntime 的调用 envelope、trace、audit 和错误映射可以用 fake provider 和受控 runner 测试。V1.8.1 后 stdio/http 已能通过 AdapterRuntime 执行，但 provider supervision、并发/限流和更完整认证会在后续节点补齐。
...
| MCP server 生命周期失控 | MCP session startup/shutdown 通过 `eva-mcp::McpSessionManager` 和 supervisor contract 表达；默认 Adapter invoke 只记录 boundary audit，不启动真实 server。 |
