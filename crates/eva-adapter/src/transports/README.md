...
这些实现证明 AdapterRuntime 的调用 envelope、trace、audit 和错误映射可以用 fake provider 和受控 runner 测试。V1.13.8 后 stdio/http、MCP stdio/HTTP JSON-RPC tool call 和 Skill workflow runner 已能通过 AdapterRuntime 进入 supervisor slot、credential session scope、concurrency/rate/circuit admission gate、bounded stream artifact 数据面、durable provider process recovery evidence、MCP compatibility matrix release gate 和 provider supervision release gate；MCP 生产 streaming/TLS/真实外部 server compatibility、OS process supervisor、OS credential vault 和 Skill output schema 深度校验会在后续节点补齐。
