...
| `mcp.rs` | MCP tool/resource/prompt | 已完成 V1.8.2 | 使用 `eva-mcp::McpJsonRpcClient`，按 manifest MCP session config 启动 stdio JSON-RPC server，并在发送前执行 tool allowlist。 |
| `skill.rs` | workflow skill | 已完成 V1.8.4 | 校验固定 skill id、`kind == workflow_skill`、`runtime_gate == normal` 和 input schema；执行受控 workflow runner 并保存 artifact evidence。 |
| `hardware.rs` | hardware device | 已完成 V1.3 | 通过 `DeviceRegistry` lease 和 `SimulatedDriver`，audit 包含 `raw_io:false`。 |
| `stdio.rs` | stdio command | 已完成 V1.8.1 | 已实现 command/args 分离、allowlist、timeout、stdout/stderr 限制、env 注入和脱敏，并接入 AdapterRuntime。 |
| `http.rs` | HTTP API | 已完成 V1.8.1 | 已实现 URL origin allowlist、method allowlist、timeout、output limit、header env 注入和输出脱敏，并接入 AdapterRuntime。 |
...
这些实现证明 AdapterRuntime 的调用 envelope、trace、audit 和错误映射可以用 fake provider 和受控 runner 测试。V1.8.4 后 stdio/http、MCP JSON-RPC tool call 和 Skill workflow runner 已能通过 AdapterRuntime 执行，但 provider supervision、并发/限流、streaming、更完整认证和 Skill output schema 深度校验会在后续节点补齐。
...
| 任意 URL / 凭据泄漏 | HTTP runner 要求 URL origin allowlist 和 method allowlist；credential env/header 注入会在输出和 audit 中脱敏。 |
