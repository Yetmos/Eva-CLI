...
| `stdio.rs` | stdio command | runner contract 已完成 | 已实现 command/args 分离、allowlist、timeout、stdout/stderr 限制；AdapterRuntime 接入留到后续步骤。 |
| `http.rs` | HTTP API | runner contract 已完成 | 已实现 URL origin allowlist、method allowlist、timeout 和 output limit；AdapterRuntime 接入留到后续步骤。 |
...
| 任意 URL / 凭据泄漏 | HTTP runner 要求 URL origin allowlist 和 method allowlist；credential env 注入仍留到 runtime 接入阶段。 |
