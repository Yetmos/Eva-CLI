...
| `runtime.rs` | 授权后 transport 执行、probe、audit | 已完成 V1.13.2 | provider invocation report 包含 request-level `TraceFields`；hardware、stdio、http、MCP JSON-RPC 和 Skill workflow runner 已接入；stdio/http/MCP/Skill 调用会先进入 provider supervisor slot，绑定 credential session scope，并释放到 process table。 |
| `supervisor.rs` | provider supervisor | 已完成 V1.13.2 | 定义 `ProviderSupervisor`、`InMemoryProviderSupervisor`、`ProviderCredentialScope`、execution slot acquire/release 和 process table mutation。 |
...
| mcp | 已完成 V1.13.2 | tool allowlist 先于 JSON-RPC；响应受 timeout 和 output limit 约束；调用进入 supervisor slot 并记录 credential session audit。 |
| skill | 已完成 V1.13.2 | runtime gate 必须为 `normal`；输入 schema、working directory、artifact path、credential/session token redaction 和 provider session scope 受控。 |
| hardware | 已完成 V1.3 | 只接受 registry lease 和 driver binding，不暴露 raw I/O。 |
| stdio | 已完成 V1.13.2 | command/args 分离，强制 allowlist，覆盖 timeout、output limit、credential/session env 注入、stdout/stderr 脱敏和 supervisor release evidence。 |
| http | 已完成 V1.13.2 | URL origin allowlist、method allowlist、timeout、output limit、credential/session header 注入、输出脱敏和 supervisor release evidence 已覆盖。 |
