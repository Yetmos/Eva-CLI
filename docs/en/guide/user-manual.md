...
Applies to: Eva-CLI `1.7.2-alpha`

This manual is for developers, testers, and documentation maintainers using
Eva-CLI from source. V1.7.2-alpha is a source alpha checkpoint: the repository
...
restricted Lua VM `on_event` execution, Lua host observability, and
`ctx.tools.call` capability binding are in place, while risky paths remain
...
| Runtime | `run --example basic` executes the V1.0 in-memory basic runtime loop through the V1.7.2 restricted Lua VM and host binding boundary. |
...
| Release checks | V1.7.2-alpha provides executable `release check/security/perf/migration` gates, including Lua VM execution and Lua host binding readiness. |
...
| Release gate | `cargo run -- release check --output json` | Prints V1.7.2 release readiness. |
...
| Release | `release check/security/perf/migration` | Run V1.7.2 release readiness, security, performance, and migration gates. | No |
...
| `release migration` | V1.5.1 to V1.7.2-alpha migration steps and compatibility policy. |
...
## Non-Goals in V1.7.2 Alpha

V1.7.2-alpha does not provide packaged installers, signed release artifacts,
...
memory/backup databases, Lua resource limits, or hot-reload generation swap.
