...
There are 123 logical changes: 2 Ready, 110 Blocked, 0 In progress, 0 In verification, and 11 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W1-L02 | Persisted TaskEnvelope | W1-L01 | Ready | W1-L01 fenced task records and CAS are Done; next persist kind, agent, input/artifact reference, idempotency key, and attempt policy, rejecting invalid kind/digest values |
| W1-L03 | Daemon lease/stale recovery | W1-L01 | Ready | W1-L01 cloneable writer generation/guard is reusable; next add PID, start token, generation, heartbeat, expiry, and safe takeover |
