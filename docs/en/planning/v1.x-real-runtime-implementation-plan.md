...
There are 123 logical changes: 1 Ready, 116 Blocked, 0 In progress, 0 In verification, and 6 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W0-L07 | Platform bundle consistency | W0-L02, W0-L06 | Ready | Capture manifests and raw stdout/stderr can be read back and rehashed; enter In progress when OS/arch/toolchain/run-id, platform artifacts, and the bundle digest are bound |
