...
There are 123 logical changes: 1 Ready, 113 Blocked, 0 In progress, 0 In verification, and 9 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W0-L10 | Revalidate downloaded evidence | W0-L06 through W0-L09 | Ready | Capture, coverage, provenance, and stable JSON/exit contracts are Done; enter In progress when upload, independent download, digest recomputation, and production-scope readback verification begin |
