...
There are 123 logical changes: 1 Ready, 112 Blocked, 0 In progress, 0 In verification, and 10 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| ID | Change summary | Direct prerequisites | State | Current blocker, next-state condition, or evidence reference |
| --- | --- | --- | --- | --- |
| W1-L01 | Durable writer ownership/CAS | W0-L10 | Ready | W0 evidence integrity and independent readback are Done; next add long-lived writer generation, record versions, CAS, atomic rename, and separate migration/runtime locks |
