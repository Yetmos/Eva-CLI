...
There are 123 logical changes: 2 Ready, 105 Blocked, 0 In progress, 0 In verification, and 16 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W1-L07 | Heartbeat/liveness | W1-L03, W1-L06 | Ready | W1-L03 and W1-L06 are Done; next renew daemon/task ownership periodically, derive live/degraded/stale status from freshness, and prevent reclaim of a fresh running attempt |
