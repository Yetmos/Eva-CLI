...
There are 123 logical changes: 2 Ready, 104 Blocked, 0 In progress, 0 In verification, and 17 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W1-L08 | ACK retry only after handler success | W1-L05 through W1-L07 | Ready | W1-L05 through W1-L07 are Done; next route replay into an owned handler, ACK only a successful terminal result, and retain durable fail/dead-letter evidence for restart continuation |
