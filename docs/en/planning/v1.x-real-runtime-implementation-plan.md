...
There are 123 logical changes: 2 Ready, 109 Blocked, 0 In progress, 0 In verification, and 12 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W1-L05 | Task handler registry | W1-L02 | Ready | W1-L02 recoverable payload contract is Done; next add a handler registry so a valid unknown kind fails stably and cannot be ACKed as success |
