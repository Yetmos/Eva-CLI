...
There are 123 logical changes: 1 Ready, 108 Blocked, 0 In progress, 0 In verification, and 14 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
