...
There are 123 logical changes: 1 Ready, 114 Blocked, 0 In progress, 0 In verification, and 8 Done. Blocked here means a strict direct prerequisite is incomplete; read-only research may run earlier, but an implementation diff cannot enter In progress until its prerequisites are satisfied.
...
| W0-L09 | JSON/exit contract | W0-L08 | Ready | Production policy and stable blocker codes are Done; enter In progress when per-gate evidence kind, provenance, remediation, and the production-blocked exit code are locked |
