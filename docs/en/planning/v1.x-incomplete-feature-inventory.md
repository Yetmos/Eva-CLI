...
| MCP | Stdio and legacy `http://` JSON-RPC remain callable with tool allowlists, timeout/output limits, and session cleanup. W4-L01 adds canonical typed `streamable_http` configuration, HTTP(S) endpoint/origin normalization, trust-root/client-auth references, same-origin redirect/allowed-origin policy, fail-closed headers, redacted audit/Debug, and the v4 manifest digest; compatibility remains a static fixture | W4-L02 TLS connector, certificate-material loading, and controlled-project-root resolution for `file:` references; W4-L03+ complete framing, the production Streamable HTTP/SSE data plane, real abort/daemon ownership, and external-server compatibility runs | P0 |
