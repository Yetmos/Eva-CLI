...
| MCP | Stdio and legacy `http://` JSON-RPC remain callable with tool allowlists, timeout/output limits, and session cleanup. W4-L01 through W4-L06 provide canonical Streamable HTTP configuration, real rustls HTTPS/mTLS, strict HTTP framing/session phases, incremental SSE, and registry-owned socket/session abort, DELETE, and bounded join. W4-L08 now adds a caller-owned numeric-loopback server transport: Host/path/Origin/media-type gates reject before body reads; HTTP/JSON framing, 64-level nesting, resources, and one socket-I/O deadline are bounded; secure random sessions execute initialize/initialized/list/call/DELETE across connections, commit only after a complete write, and are cleared while shutdown actively aborts the current socket. Explicit read-only tools and executable schemas gate calls before the handler. Real TCP lifecycle, deep-input, slow-drip, early-header and `Accept: application/json;q=0` denial, notification 202, exhausted-deadline, shutdown-race, failed-write, and limit-fallback/no-state-commit regressions pass in `eva-mcp` 108/108. Compatibility remains a static fixture | W4-L02 still needs one monotonic deadline across DNS and every I/O phase, real slow-drip/write-timeout coverage, Unix/Windows handle-based traversal that closes ancestor-replacement TOCTOU, and native three-platform evidence. W4-L07 still needs daemon ownership; W4-L09/L10 still need an executed compatibility manifest and external-server matrix. W4-L08's synchronous handler is a caller-trusted boundary and does not claim forced cancellation, inbound TLS/auth, or remote exposure | P0 |
