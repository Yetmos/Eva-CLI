...
W3-L09 local macOS update (2026-07-27): the explicitly injected `MacosKeychainCredentialVault` implements current-user Keychain lookup for `vault://keychain/<service>/<account>` with strict reference and redaction regressions; the default production vault remains fail-closed. It is not an approved Keychain/KMS authority and does not replace cross-UID, complete TOCTOU/leak scanning, or three-platform production evidence.
