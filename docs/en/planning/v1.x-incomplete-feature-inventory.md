...
| MCP | Stdio and legacy `http://` JSON-RPC remain callable with tool allowlists, timeout/output limits, and session cleanup. W4-L01 adds canonical typed `streamable_http` configuration, HTTP(S) endpoint/origin normalization, trust-root/client-auth references, same-origin redirect/allowed-origin policy, fail-closed headers, redacted audit/Debug, and the v4 manifest digest. W4-L02 now wires real rustls HTTPS, system/custom trust roots, chain/hostname/SNI/validity checks, mTLS, `env:`/declared `vault://` material, controlled-project-root `file:` reads, and an end-to-end HTTPS MCP call; compatibility remains a static fixture | W4-L02 still needs one monotonic deadline across DNS and every I/O phase, real slow-drip/write-timeout coverage, Unix/Windows handle-based traversal that closes ancestor-replacement TOCTOU, and native three-platform evidence. W4-L03+ complete framing, the production Streamable HTTP/SSE data plane, real abort/daemon ownership, and external-server compatibility runs remain incomplete | P0 |
