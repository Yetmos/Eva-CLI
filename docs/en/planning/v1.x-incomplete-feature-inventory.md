...
| MCP | Stdio and legacy `http://` JSON-RPC remain callable with tool allowlists, timeout/output limits, and session cleanup. W4-L01 through W4-L06 provide canonical Streamable HTTP configuration, real rustls HTTPS/mTLS, strict HTTP framing/session phases, incremental SSE, and registry-owned socket/session abort, DELETE, and bounded join. W4-L02 now applies one absolute deadline across DNS/connect/handshake/write/response, caps DNS workers/addresses, and closes supported-platform ancestor-replacement TOCTOU with Linux/macOS `openat` and Windows root-handle/final-path same-handle reads. Real blocked-write, slow-drip, DNS-capacity, and Windows rename/move-out regressions pass. W4-L08 adds the caller-owned numeric-loopback server transport with pre-body Host/path/Origin/media gates, bounded HTTP/JSON/resources, write-before-session-commit, active shutdown, explicit read-only tools, and executable schemas. `eva-mcp` passes 113/113 together with workspace tests/Clippy and the Windows GNU cross-check. Compatibility remains a static fixture | W4-L02's code gap is closed but still needs native Linux/macOS/Windows CI evidence on the same commit. W4-L07 still needs daemon ownership; W4-L09/L10 still need an executed compatibility manifest and external-server matrix. W4-L08's synchronous handler is a caller-trusted boundary and does not claim forced cancellation, inbound TLS/auth, or remote exposure | P0 |
