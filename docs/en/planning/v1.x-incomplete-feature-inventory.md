...
| MCP | Stdio and legacy `http://` JSON-RPC remain callable with tool allowlists, timeout/output limits, and session cleanup. W4-L01 adds canonical typed `streamable_http` configuration, HTTP(S) endpoint/origin normalization, trust-root/client-auth references, same-origin redirect/allowed-origin policy, fail-closed headers, redacted audit/Debug, and the v4 manifest digest. W4-L02 wires real rustls HTTPS, system/custom trust roots, chain/hostname/SNI/validity checks, mTLS, `env:`/declared `vault://` material, controlled-project-root `file:` reads, and an end-to-end HTTPS MCP call; W4-L03 provides bounded Content-Length/chunked/keep-alive/202/204/invalid-header framing; W4-L04 adds strict initialize/version fields, enforced phases, `Mcp-Session-Id`, later POST/GET control headers, 404/mismatch/provisional-session cleanup, and DELETE/stateless shutdown. W4-L05 adds pull-based incremental SSE, all three body sources, GET/POST streams, strict direct request-ID correlation, retained interleaved peer messages, disconnect/UTF-8/limit handling, and redacted Debug. W4-L06 adds registry-owned reader/socket/session cleanup ordered as socket shutdown, DELETE, then bounded join, with cleanup-pending retry preserving failed ownership. A real HTTP fixture plus plaintext/TLS abort regressions prove zero dangling sessions/readers, with 83 passing MCP regressions. Compatibility remains a static fixture | W4-L02 still needs one monotonic deadline across DNS and every I/O phase, real slow-drip/write-timeout coverage, Unix/Windows handle-based traversal that closes ancestor-replacement TOCTOU, and native three-platform evidence. W4-L07 still needs daemon ownership, while W4-L08/L09/L10 still need a real server transport, executed compatibility manifest, and external-server matrix | P0 |
