...
This page is the English entry for the full Eva-CLI implementation plan. The full-detail source remains the Simplified Chinese document: [当前项目从零到完整实现实施计划.md](../../zh-CN/planning/当前项目从零到完整实现实施计划.md).
...
- The Rust workspace contains 20 crates.
...
- V1.1 external capability ecosystem is implemented: `eva-adapter` provides authorized handles, registry, router, probe, and controlled MCP/Skill boundaries; later V1.8 slices upgrade stdio/http, MCP JSON-RPC, and Skill workflow runners into controlled execution paths; `eva-discovery` emits non-authorizing candidates; `eva-cli` exposes `adapter`, `mcp`, `skill`, and `discovery` commands.
...
- V1.5 release hardening is implemented: root/workspace version is `1.5.1`, `eva-release` provides cross-platform readiness gates, security review findings, performance budgets, migration guide, and compatibility policy; `eva-cli` exposes `release check`, `release security`, `release perf`, and `release migration`.
...
V1.5 is complete as a source release checkpoint for controlled external capability visibility, diagnostics, request-scoped memory/knowledge context, non-authorizing hardware access planning, plan-first backup/lifecycle operations, and executable release-hardening gates. Real Lua VM sandboxing, durable task recovery, packaged installers, signed release artifacts, real stdio/http/MCP process execution, durable memory storage, real hardware I/O, destructive restore, and real supervisor process management remain later-version scope.
