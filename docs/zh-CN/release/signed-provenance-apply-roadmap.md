...
| --- | --- | --- | --- | --- | --- |
| P0-001 | 创建本实施台账，并注册到 docs/site 索引 | `docs/en/release/signed-provenance-apply-roadmap.md`、`docs/zh-CN/release/signed-provenance-apply-roadmap.md`、`docs/_i18n/manifest.json`、docs/site indexes | `scripts/build-site-i18n.ps1`；`scripts/validate-i18n.ps1`；`scripts/validate-version-management.ps1` | In Progress | pending |
| P1-001 | 为 GHCR release job 增加 OIDC/attestation 权限和 Buildx SBOM/provenance 设置 | `.github/workflows/release.yml` | workflow diff review；`scripts/validate-version-management.ps1` | Planned | pending |
| P1-002 | 扩展 `package-ghcr.json`，记录 provenance 和 SBOM 字段，同时保留旧字段 | `.github/workflows/release.yml`、release docs | `scripts/validate-version-management.ps1`；JSON 字段审查 | Planned | pending |
