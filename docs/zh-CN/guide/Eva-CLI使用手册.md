...
`--version` / `version --output json` 包含 `mcp_http_auth_v1.13.6` 和
`mcp_compat_matrix_v1.13.7` 时，表示 runtime 已支持带 auth/session header 的
受限 MCP `http://` JSON-RPC transport 边界，并已具备 repo-local MCP
