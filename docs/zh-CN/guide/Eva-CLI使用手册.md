...
适用版本：Eva-CLI `1.7.1-alpha`

本文面向从源码使用 Eva-CLI 的开发者、测试者和文档维护者。当前版本是
V1.7.1-alpha 源码 alpha 检查点：仓库可以编译，CLI 命令面可以执行，durable
EventBus redrive、可重启读取的 task snapshot、durable audit record、artifact metadata evidence、runtime recovery scanner、redrive checkpoint、recovery audit smoke、durable diagnostics 和受限 Lua VM `on_event` 真实执行边界已经落地，但高风险路径仍以受控诊断、in-memory 示例闭环、plan-first 和发布门禁为主。
...
| 运行时 | `run --example basic` 通过 V1.7.1 受限 Lua VM 边界执行 V1.0 in-memory basic runtime 闭环。 |
| 外部能力 | Adapter、MCP、Skill、Discovery 当前提供受控诊断面，不启动真实 provider。 |
| 风险动作 | 硬件绑定、恢复、升级和生命周期切换保持 plan-first，不执行 destructive apply。 |
| 发布检查 | V1.7.1-alpha 提供 `release check/security/perf/migration` 可执行门禁，包含 Lua VM execution readiness。 |
...
| 发布门禁 | `cargo run -- release check --output json` | 输出 V1.7.1 release readiness。 |
...
| Release | `release check/security/perf/migration` | 执行 V1.7.1 发布准备、安全、性能和迁移门禁。 | 否 |
...
| `release migration` | V1.5.1 到 V1.7.1-alpha 的迁移步骤和兼容性策略。 |
...
- Lua `ctx.tools` / `ctx.host`、资源限制和 hot-reload generation swap。
