> Language: 简体中文
> English default entry: ../en/module-partitioning.md
...
本文把现有 Eva-CLI 架构方案落成模块架构边界。当前仓库仍以架构文档为主，`src/` 和 `crates/` 还没有实际 Rust 模块，所以本文描述的是目标 workspace 形态、crate 边界、依赖方向、运行时交接点和职责归属，而不是现有代码的反向说明，也不是实施排期。
...
- EventBus、Scheduler、AgentRuntime、Lua host、AdapterRegistry、MCP server 必须在职责和依赖方向上保持可分离。
- 模块命名应对应稳定架构概念，不对应临时实施阶段。

## 3. 模块总览

![Eva-CLI 模块划分总览](../assets/module-partition-overview.zh-CN.svg)

推荐使用 Rust workspace 承载稳定边界。每个 crate 是一个职责域，crate 内部再使用 module 划分实现细节。这里给出的是架构边界图，不是交付顺序。
...
| 状态归属变得模糊 | Agent、持久业务、事件恢复、Adapter、lifecycle 状态分别归属对应模块 |
| 具体 transport 向内泄漏 | provider、MCP、shell、HTTP、skill、hardware 细节只保留在 adapter/service crate |
...
这套划分保持 Eva-CLI 的架构边界清晰：Rust 托管可信运行时边界，Lua 承载可替换业务行为，EventBus 与 Scheduler 负责协作，Adapter 和服务 crate 负责副作用，`eva-runtime` 是唯一组合具体实现的位置。
