...
| MCP | Stdio 与 legacy `http://` JSON-RPC、tool allowlist、timeout/output limit 和 session cleanup 已可调用；W4-L01 已接入 canonical `streamable_http` 强类型配置、HTTP(S) endpoint/origin 规范化、trust-root/client-auth 引用、same-origin redirect/allowed-origin、header fail-closed、脱敏 audit/Debug 与 v4 manifest digest，compatibility 仍是静态 fixture | W4-L02 的 TLS connector、证书材料读取及 `file:` 相对受控项目根解析，W4-L03 之后的完整 framing、生产 Streamable HTTP/SSE 数据面、真实 abort/daemon ownership 和外部 server compatibility 实跑 | P0 |
