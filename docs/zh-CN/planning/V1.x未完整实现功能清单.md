...
| MCP | Stdio 与 legacy `http://` JSON-RPC、tool allowlist、timeout/output limit 和 session cleanup 已可调用；W4-L01 至 W4-L06 已接入 canonical Streamable HTTP 配置、真实 rustls HTTPS/mTLS、严格 HTTP framing/session phase、增量 SSE，以及 registry-owned socket/session abort、DELETE 与 bounded join。W4-L02 现以同一绝对 deadline 覆盖 DNS/connect/handshake/write/response，限制 DNS worker/address，并用 Linux/macOS `openat` 和 Windows root-handle/final-path 同句柄读取关闭受支持平台的祖先替换 TOCTOU；真实 blocked-write、slow-drip、DNS capacity 与 Windows rename/move-out 回归已通过。W4-L08 已接入调用方托管的数值 loopback server transport：Host/path/Origin/media-type 在 body 前拒绝，HTTP/JSON/64 层深度/资源/socket-I/O deadline 有界，安全随机 session 跨连接执行 initialize/initialized/list/call/DELETE，完整写回后才提交状态，shutdown 主动中止当前 socket 并清零 session；显式只读 tool 与可执行 schema 在 handler 前门禁。`eva-mcp` 113/113 与 workspace tests/Clippy、Windows GNU cross-check 通过。compatibility 仍是静态 fixture | W4-L02 代码缺口已关闭，仍需同一提交的 Linux/macOS/Windows 原生 CI evidence；W4-L07 仍需 daemon ownership；W4-L09/L10 仍需实跑 compatibility manifest 与外部 server matrix。W4-L08 的同步 handler 是调用方受信任边界，不宣称可强制取消、入站 TLS/auth 或远程暴露 | P0 |
