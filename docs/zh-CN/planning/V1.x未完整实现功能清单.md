...
| MCP | Stdio 与 legacy `http://` JSON-RPC、tool allowlist、timeout/output limit 和 session cleanup 已可调用；W4-L01 已接入 canonical `streamable_http` 强类型配置、HTTP(S) endpoint/origin 规范化、trust-root/client-auth 引用、same-origin redirect/allowed-origin、header fail-closed、脱敏 audit/Debug 与 v4 manifest digest。W4-L02 已接入真实 rustls HTTPS、系统/自定义 trust root、证书链/hostname/SNI/有效期、mTLS、`env:`/已声明 `vault://` 材料和 canonical 受控项目根下的 `file:` 读取；W4-L03 已接入有界 Content-Length/chunked/keep-alive/202/204/invalid-header framing；W4-L04 已接入严格 initialize/version 字段解析、显式 phase、`Mcp-Session-Id`、后续 POST/GET 控制 header、404/mismatch/provisional-session cleanup 与 DELETE/stateless shutdown；W4-L05 已接入 pull-based 增量 SSE、三类 body source、GET/POST stream、严格顶层 request ID 关联、交错 peer message 保留、断流/UTF-8/上限和脱敏 Debug；W4-L06 新增 registry-owned reader/socket/session cleanup，按 socket shutdown→DELETE→bounded join 顺序释放，并以 cleanup-pending 重试守住失败状态。真实 HTTP fixture 与明文/TLS abort 回归证明 dangling session/reader 为 0，MCP 83 项回归通过。compatibility 仍是静态 fixture | W4-L02 仍需覆盖 DNS 与各 I/O 阶段的单调总 deadline、真实 slow-drip/write-timeout、关闭 ancestor replacement TOCTOU 的 Unix/Windows handle-based 读取及三平台原生 evidence；W4-L07 仍需 daemon ownership，W4-L08/L09/L10 仍需真实 server transport、实跑 compatibility manifest 与外部 server matrix | P0 |
