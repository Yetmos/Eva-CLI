...
W3-L08 macOS 本机状态更新（2026-07-27）：真实子进程回归已证明 `ProviderRunAsIdentity::Current` 保持当前 daemon EUID/EGID；macOS 显式 Unix identity 继续在 spawn 前 fail-closed。该结果不替代跨 UID/root handoff、目录 TOCTOU、真实 vault authority 或三平台 production evidence。

W3-L09 macOS 本机状态更新（2026-07-27）：显式注入的 `MacosKeychainCredentialVault` 已实现 `vault://keychain/<service>/<account>` 的当前用户 Keychain 查询和严格 ref/脱敏回归；默认生产 vault 继续 fail-closed。它不构成获批 Keychain/KMS authority，也不替代跨 UID、完整 TOCTOU/leak scan 或三平台 production evidence。
