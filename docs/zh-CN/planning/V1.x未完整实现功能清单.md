...
| Upgrade 与 Service Manager | W2-L01/L02 已完成；Windows Service、systemd、launchd Adapter 代码已实现并处于受控生命周期验证中，仍保留 fake adapter 作为单测边界 | W2-L06 至 W2-L10：Service CLI、daemon entrypoint、开机恢复、三平台 destructive harness 和 production evidence；真实 candidate 流量切换仍未完成 | P0 |
...
| `platform_service_manager_test_environment` | 受控 Windows/Linux/macOS service host、提权/登录权限 | W2-L03 至 L05 的真实 host transcript，W2-L06 至 L10 的 CLI/entrypoint/recovery/harness/gate，以及 W3-L11 production evidence |
