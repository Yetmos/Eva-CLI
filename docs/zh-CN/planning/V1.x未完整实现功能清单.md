...
| Provider 与 Capability 执行 | W3-L01 至 W3-L07 已完成：真实 PID/CAS、process-group/Job cleanup、restart budget、stdio/MCP 登记和跨进程 admission 均已存在；run `29597430669` 的 Linux/macOS/Windows evidence 成功 | W3-L08 run-as 用户隔离、W3-L09 credential vault、W3-L10 daemon-owned 长驻 supervisor、W3-L11 三平台 production evidence（其中 Linux/macOS 原生 crash-loop/daemon-kill 尚缺）；TLS/streaming 仍属于 W4 | P0 |
...
| Upgrade 与 Service Manager | W2-L01/L02/L06 已完成；`eva service install/status/start/stop/restart/uninstall` 已接线，Fake 仅在显式 `--dev` 下可用，并以项目/服务隔离的开发状态文件支持跨 CLI 进程生命周期；Windows Service、systemd、launchd Adapter 代码仍处于受控生命周期验证中 | W2-L03 至 L05 的真实 host transcript，以及 W2-L07 至 L10 的 daemon entrypoint、开机恢复、三平台 destructive harness 和 production evidence；真实 candidate 流量切换仍未完成 | P0 |
...
| `platform_service_manager_test_environment` | 受控 Windows/Linux/macOS service host、提权/登录权限 | W2-L03 至 L05 的真实 host transcript，W2-L07 至 L10 的 entrypoint/recovery/harness/gate，以及 W3-L11 production evidence；W2-L06 CLI 代码与契约已在仓库内验证 |
