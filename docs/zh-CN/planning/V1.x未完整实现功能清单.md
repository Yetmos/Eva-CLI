...
| --- | --- | --- | --- |
| Release evidence | `release check` 聚合编译进代码的 alpha checklist object、静态 MCP compatibility fixture、固定 performance observation 和可选 operator evidence file | 将 gate 绑定到带时间、来源和 provenance 的真实 CI/平台执行证据，并区分声明、fixture 与观测结果 | P0 |
...
| Gate 过度可信 | 静态 pass object 和固定 performance 值容易被理解为实测证据 | 生产 gate 记录 source、timestamp、commit、environment 和 artifact digest |
