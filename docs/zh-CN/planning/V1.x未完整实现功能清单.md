...
| Runtime、Task 与 Scheduler | 前台 daemon、带 heartbeat 与 dead-expired takeover 的 fenced writer/daemon lease、绑定完整 identity 的 PID status、文件 mailbox、durable task/event/provider snapshot、启动恢复分类、retry routing/ACK evidence 和 daemon-side Agent control state | 后台服务、真实 task/Agent worker 消费、task/worker heartbeat 与 degraded/stale 分类、开机恢复和副作用恢复 | P0 |
| Provider 与 Capability 执行 | 每次调用启动的 stdio/MCP/Skill process runner 已接中央真实 PID/process-boundary 登记、group/Job cleanup、durable attempt budget、指数 backoff/jitter、稳定窗口和 daemon generation recovery；另有明文 HTTP、allowlist、单进程 admission 与脱敏 | 支持 TLS 的 Provider HTTP client、daemon-owned 长驻 supervision、跨进程 admission、用户隔离、credential vault，以及 native Linux/macOS crash-loop/daemon-kill evidence | P0 |
