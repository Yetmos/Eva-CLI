...
当前共有 123 个逻辑修改：0 个可开始、101 个已阻塞、0 个进行中、1 个验证中、21 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。

| Work Package | 逻辑项 | 可开始 | 未开始 | 已阻塞 | 进行中 | 验证中 | 已完成 | 已延期 | 已移出范围 | 聚合状态 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| W0 | 10 | 0 | 0 | 0 | 0 | 0 | 10 | 0 | 0 | Complete |
| W1 | 12 | 0 | 0 | 0 | 0 | 1 | 11 | 0 | 0 | In progress |
...
| W1-L12 | 三平台进程级 evidence | W0-L10、W1-L04、W1-L08、W1-L10、W1-L11 | 验证中 | `tests/background_runtime.rs` 的显式 W1 suite 用真实 background daemon 覆盖受控 shutdown、强杀后 dead-but-fresh lease 拒绝、自然到期后更高 generation 接管、幂等 running task attempt 2 恢复，以及 Committed 后/task 终态前与 Prepared 后/handler 返回前两个非幂等崩溃窗口；外部效果文件在调用前后 `sync_all`，重启后分别精确复用 ledger digest/size 或稳定进入 operator reconciliation，重复重启仍不再调用 handler。测试 handler、缩短 TTL/匹配 heartbeat 和 committed pause 只在 debug build、显式 `--dev` 与 harness 目录环境变量同时存在时注册，默认/Release registry 仍只有 `runtime.echo`。`scripts/validate-runtime-process-evidence.ps1` 从可信 commit/run/attempt 严格回读固定五场景、W0 `measurement` envelope、真实 command capture 与 stdout/stderr 摘要，拒绝缺失/重复/额外/失败场景、混合平台/提交、未知文件和 reparse path；`.github/workflows/ci.yml` 的 Windows/Linux/macOS producer 独立上传，aggregate job 精确要求三平台后再发布 verified bundle。当前 Windows 本地验证为 W1 suite 1/1、既有 background 6/6、runtime 104/104、observability 21/21、evidence 正/负向合同、Runtime/root Clippy `-D warnings` 与 Release build 通过；OTLP fake collector 已改为 exporter 返回后显式 stop/join，目标用例连续 50 次通过。完成条件是本次提交推送后 GitHub-hosted 三平台 artifact 聚合实际通过 |
