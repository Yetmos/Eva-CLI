...
当前共有 123 个逻辑修改：6 个可开始、83 个已阻塞、0 个进行中、7 个验证中、29 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | 新增 `eva-storage::FileSystemProviderAdmissionTable`，以每个 Adapter 的稳定文件锁作为线性化点，持久化 reservation、容量检查、同 session 幂等、完成释放和过期回收；`ProviderExecutionSlot` 现在携带 reservation identity，durable supervisor 的 restart 会恢复唯一匹配 reservation，complete/renew 均按 reservation ID + session 双身份 fence，旧 slot 不能释放 successor。父测试启动两个真实子进程竞争 `max_concurrency=1`，确认仅一个成功且持久表仅保留一个 reservation；错误 session 与过期 lease 均拒绝。`eva-storage` admission、`eva-adapter` supervisor/runtime 测试和 `-D warnings` Clippy 通过。supervisor 周期 renew、crash reclaim 与对应 W0 evidence 尚未完成，需受控进程 harness/evidence 后才能标记已完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 验证中 | 新增 `eva-config::ConfigGeneration`，对已加载且校验通过的项目配置生成稳定环境标识与内容 digest，并拒绝零 generation；同一配置重复计算保持一致，新增 runtime environment 变化会同时改变 environment 与 digest 的回归。`eva-config` 57 项测试、fmt 与 Clippy 通过；profile/user/env 分层 merge、覆盖冲突诊断和原子 reload 尚未接线 |
...
| W8-L07 | Runtime-wide OTel lifecycle | W1-L12 | 验证中 | 新增 runtime-owned `RuntimeObservabilityLifecycle`，统一 start、pipeline access、flush/shutdown 状态门禁和 continuity report；daemon 现在从 durable recovery 前创建 owner，让 recovery 与 startup verification 复用同一 pipeline，并在正常/错误退出时于 lease release 前 shutdown，audit 标识生命周期持有到 shutdown。真实启动 smoke、control shutdown、submit/cancel observability 回归与 runtime Clippy 通过。scheduler/control/memory 等记录函数仍有短生命周期 open，且尚未覆盖真实 collector 长驻 flush evidence |
