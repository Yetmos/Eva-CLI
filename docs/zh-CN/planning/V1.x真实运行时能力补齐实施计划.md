...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 进行中 | 新增 `eva-config::ConfigGeneration`，对已加载且校验通过的项目配置生成稳定环境标识与内容 digest，并拒绝零 generation；同一配置重复计算保持一致，新增 runtime environment 变化会同时改变 environment 与 digest 的回归。`eva-config` 57 项测试、fmt 与 Clippy 通过；profile/user/env 分层 merge、覆盖冲突诊断和原子 reload 尚未接线 |
