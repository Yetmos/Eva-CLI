...
| W8-L02 | Durable schedule model | W1-L01、W1-L12 | 验证中 | 新增 `eva-memory::FileSystemScheduleStore`，持久化 next-run、owner lease、expiry 和 generation，支持 upsert/claim/complete 及过期租约接管；重启后从 schedule 文件恢复。`eva-memory` 测试、fmt 与 Clippy 通过；尚未接入 daemon 长驻 scheduler、renew heartbeat 与生产 retention policy |
