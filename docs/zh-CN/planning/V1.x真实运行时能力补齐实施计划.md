...
| W8-L02 | Durable schedule model | W1-L01、W1-L12 | 验证中 | `eva-memory::FileSystemScheduleStore` 持久化 next-run、owner lease、expiry 和 generation，支持 upsert/claim/complete、过期租约接管及重启恢复；本次增加 per-schedule create-new lock、60s 崩溃残留锁回收和双线程同时 claim 单赢家回归。`eva-memory` 33/33、fmt 与 Clippy 通过；尚未接入 daemon 长驻 scheduler、renew heartbeat 与生产 retention policy |
