...
进度快照基于 2026-07-15 的 main 提交 82f7416。表格中的“当前行为”只说明可复用的 alpha/library/test 基线，不等于目标逻辑已经完成。
...
| W0-L03 | eva-cli/src/run/release_cmd.rs、eva-release/src/checklist.rs | CLI 接受四类可选 evidence 文件，默认仍可 ready | 增加统一 evidence manifest 与 alpha/production scope；旧参数先转换 envelope 再聚合 | W0-L01、W0-L02 | production 缺 manifest 非零退出；alpha 与 production 状态互不冒充 |
...
| W1-L04 | daemon.rs、daemon_cmd.rs | background=false 以外直接 unsupported | 实现父子后台启动、ready handshake、真实 PID 返回与失败清理 | W1-L03 | 父进程退出后 daemon 可 status/shutdown；失败无残留 pid/lock |
...
| W1-L12 | eva-runtime/tests/background_runtime.rs（新）、CI workflow | 只有线程内/前台恢复测试 | 建立真实子进程 harness，覆盖 background、SIGKILL、stale lock、restart、effect 去重并产 W0 envelope | W0-L10、W1-L04、W1-L08、W1-L10、W1-L11 | 三平台进程级 evidence；缺任一场景时 W1 gate 阻断 |
...
| W2-L06 | eva-cli/src/run/service_cmd.rs（新）、run.rs | CLI 没有真实 service lifecycle 命令 | 增加 service install/status/start/stop/restart/uninstall；Fake 仅显式 dev mode | W2-L03 至 W2-L05 | JSON 固定 mutation_executed、production_adapter、平台错误与幂等状态 |
...
当前共有 123 个逻辑修改：1 个可开始、122 个已阻塞、0 个进行中、0 个验证中、0 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。

| Work Package | 逻辑项 | 可开始 | 未开始 | 已阻塞 | 进行中 | 验证中 | 已完成 | 已延期 | 已移出范围 | 聚合状态 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| W0 | 10 | 1 | 0 | 9 | 0 | 0 | 0 | 0 | 0 | Open |
...
| --- | --- | --- | --- | --- |
| W0-L01 | 统一 EvidenceEnvelope | 无新增前置 | 可开始 | 开始 schema/测试差异后进入进行中；实现完成后以四类往返/拒绝测试进入验证中 |
| W0-L02 | Provenance 与 subject digest 校验 | W0-L01 | 已阻塞 | W0-L01 已完成 |
| W0-L03 | CLI evidence manifest 与 scope | W0-L01、W0-L02 | 已阻塞 | 两项前置已完成 |
| W0-L04 | 静态声明/fixture 分类 | W0-L01、W0-L03 | 已阻塞 | production scope contract 已完成 |
| W0-L05 | 性能预算与实测分离 | W0-L01、W0-L04 | 已阻塞 | evidence kind 已进入 gate |
| W0-L06 | 真实命令 evidence capture | W0-L01、W0-L02 | 已阻塞 | envelope 与 digest verifier 已完成 |
| W0-L07 | 平台 bundle 与一致性聚合 | W0-L02、W0-L06 | 已阻塞 | capture 输出可验证 |
| W0-L08 | Coverage/freshness/trust policy | W0-L03 至 W0-L07 | 已阻塞 | 所有 producer 与分类语义完成 |
| W0-L09 | JSON/exit contract | W0-L08 | 已阻塞 | production gate policy 稳定 |
| W0-L10 | 回读 evidence 后重验 | W0-L06 至 W0-L09 | 已阻塞 | capture、coverage 与 contract 全完成 |

### W1 进度

| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W1-L01 | Durable writer ownership/CAS | W0-L10 | 已阻塞 | W0 完成并能记录存储竞争 evidence |
| W1-L02 | 持久 TaskEnvelope | W1-L01 | 已阻塞 | storage CAS contract 完成 |
| W1-L03 | Daemon lease/stale recovery | W1-L01 | 已阻塞 | writer ownership 可复用 |
| W1-L04 | Background spawn/handshake | W1-L03 | 已阻塞 | lease 与 stale 判定完成 |
| W1-L05 | Task handler registry | W1-L02 | 已阻塞 | 可恢复 payload contract 完成 |
| W1-L06 | Worker claim/execute loop | W1-L03 至 W1-L05 | 已阻塞 | lease、background、handler 全完成 |
| W1-L07 | Heartbeat/liveness | W1-L03、W1-L06 | 已阻塞 | worker owner 可续租 |
| W1-L08 | Handler 成功后再 ACK retry | W1-L05 至 W1-L07 | 已阻塞 | 有 owner 的 handler lifecycle 完成 |
| W1-L09 | Durable effect ledger | W1-L01、W1-L02、W1-L05 | 已阻塞 | storage/task/handler contract 完成 |
| W1-L10 | Crash recovery 语义 | W1-L07 至 W1-L09 | 已阻塞 | liveness、retry、effect ledger 完成 |
| W1-L11 | Shutdown/drain ownership | W1-L06、W1-L07、W1-L10 | 已阻塞 | worker/recovery 状态机完成 |
| W1-L12 | 三平台进程级 evidence | W0-L10、W1-L04、W1-L08、W1-L10、W1-L11 | 已阻塞 | 所有 background runtime 行为可执行 |

### W2 进度

| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W2-L01 | 完整 ServiceManager contract | W1-L12 | 已阻塞 | W1 process substrate 完成 |
| W2-L02 | Host executor/factory | W2-L01 | 已阻塞 | typed lifecycle contract 完成 |
| W2-L03 | Windows Service Adapter | W2-L02、W1-L04 | 已阻塞 | executor 与 background entrypoint 完成 |
| W2-L04 | systemd Adapter | W2-L02、W1-L04 | 已阻塞 | executor 与 background entrypoint 完成 |
| W2-L05 | launchd Adapter | W2-L02、W1-L04 | 已阻塞 | executor 与 background entrypoint 完成 |
| W2-L06 | Service CLI | W2-L03 至 W2-L05 | 已阻塞 | 三平台 Adapter code 完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W3-L01 | Provider restart/identity/vault config | W1-L12 | 已阻塞 | W1 owner/recovery substrate 完成 |
| W3-L02 | 真实 PID/process record CAS | W1-L01、W3-L01 | 已阻塞 | storage ownership 与 manifest 完成 |
| W3-L03 | OS process group/Job backend | W3-L02 | 已阻塞 | durable process identity 完成 |
| W3-L04 | Stdio/MCP 接中央 spawn/register | W3-L03 | 已阻塞 | platform backend 可持有真实 handle |
| W3-L05 | Group kill/reap/orphan scan | W3-L04 | 已阻塞 | 所有 spawn 已经过 backend |
| W3-L06 | Durable restart controller | W3-L02、W3-L05 | 已阻塞 | process record 与 cleanup 完成 |
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 已阻塞 | durable CAS/reservation 基础完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 已阻塞 | W1 owner/cancel/recovery 完成 |
| W6-L02 | Immutable config generation | W6-L01 | 已阻塞 | canonical merged config 完成 |
| W6-L03 | Long-lived config watcher | W6-L02 | 已阻塞 | candidate generation contract 完成 |
| W6-L04 | Reload preflight/fail-safe | W6-L03 | 已阻塞 | watcher event lifecycle 完成 |
| W6-L05 | Atomic config/policy/route swap | W6-L04、W1-L06 | 已阻塞 | validated candidate 与 request ownership 完成 |
| W6-L06 | Durable generation recovery | W6-L05 | 已阻塞 | atomic swap boundary 完成 |
| W6-L07 | Cancellable discovery deadline | W1-L12、W6-L02 | 已阻塞 | cancel owner 与 discovery config generation 完成 |
| W6-L08 | Real PATH resolution/probe | W6-L07 | 已阻塞 | cancellable source contract 完成 |
| W6-L09 | Registry TLS/auth/protocol client | W6-L07、DEC-02 | 已阻塞 | source contract 与 registry 决策完成 |
| W6-L10 | Durable TTL discovery cache | W6-L07 | 已阻塞 | source result/failure semantics 完成 |
| W6-L11 | Complete mutation policy inventory | W6-L01 | 已阻塞 | layered config/policy inputs 稳定 |

### W7 进度

| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W7-L01 | Real hardware typed config | W1-L12 | 已阻塞 | W1 lifecycle substrate 完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W8-L01 | Remove operator demo seed | W1-L12 | 已阻塞 | W1 durable owner 完成，确保空库/并发语义稳定 |
| W8-L02 | Durable schedule model | W1-L01、W1-L12 | 已阻塞 | W1 storage ownership 与 process evidence 完成 |
| W8-L03 | Schedule claim/reclaim store | W1-L12、W8-L02 | 已阻塞 | ownership 与 schedule state machine 完成 |
| W8-L04 | Daemon memory/retrieval worker | W8-L03 | 已阻塞 | durable scheduling 完成 |
...
| W8-L07 | Runtime-wide OTel lifecycle | W1-L12 | 已阻塞 | daemon background lifecycle 完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W9-L01 | Canonical package metadata | W0-L10 | 已阻塞 | artifact provenance/evidence integrity 完成 |
| W9-L02 | Homebrew formula | W9-L01 | 已阻塞 | canonical metadata 完成 |
| W9-L03 | Winget manifests | W9-L01 | 已阻塞 | canonical metadata 完成 |
| W9-L04 | deb/Apt metadata | W9-L01 | 已阻塞 | canonical metadata 完成 |
| W9-L05 | Credential-free validator run | W9-L02 至 W9-L04 | 已阻塞 | 三类 generator 完成 |
