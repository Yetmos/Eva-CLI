...
| W2-L07 | daemon_cmd.rs、daemon.rs、service_cmd.rs、eva-lifecycle/service_entry.rs 与三平台 Adapter | 生产 definition 已绑定 canonical hidden entrypoint；service manager 直接托管当前 daemon 进程，PID/state/lease owner 一致，stop token 复用既有 drain/shutdown | 固定无 shell native argv、working directory、service identity 与 no-respawn 契约，并对配置/入口 drift fail closed | W1-L04、W1-L11、W2-L06 | daemon PID 与 service manager PID 一致；stop 经过 generation-bound drain/shutdown；身份或 argv drift 在启动/控制前拒绝 |
...
当前共有 123 个逻辑修改：1 个可开始、56 个已阻塞、0 个进行中、11 个验证中、55 个已完成。这里的“已阻塞”表示严格直接前置或明确的外部验收边界尚未满足，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。

| Work Package | 逻辑项 | 可开始 | 未开始 | 已阻塞 | 进行中 | 验证中 | 已完成 | 已延期 | 已移出范围 | 聚合状态 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| W0 | 10 | 0 | 0 | 0 | 0 | 0 | 10 | 0 | 0 | Complete |
| W1 | 12 | 0 | 0 | 0 | 0 | 0 | 12 | 0 | 0 | Complete |
| W2 | 10 | 0 | 0 | 3 | 0 | 3 | 4 | 0 | 0 | In progress + EXT-01 |
| W3 | 11 | 0 | 0 | 1 | 0 | 3 | 7 | 0 | 0 | Open + EXT-01 |
| W4 | 10 | 1 | 0 | 3 | 0 | 2 | 4 | 0 | 0 | Open + EXT-03 |
...
| W2-L07 | Daemon service entrypoint | W1-L04、W1-L11、W2-L06 | 已完成 | `ServiceManagerEntryPoint::for_daemon` 将 canonical executable、逐项 native argv、canonical project working directory 与稳定 SHA-256 identity 绑定为隐藏的 `daemon __service-entry`；Service CLI 只为 production definition 安装该入口，Fake 保持原开发边界。Windows `binPath`、systemd `ExecStart`/`WorkingDirectory` 与 launchd `ProgramArguments`/`WorkingDirectory` 都保留同一 argv/identity，并在已安装配置、managed marker 或 plist 漂移时 fail closed。隐藏 CLI 入口重算并校验 project、host kind、service name 与 identity 后，进入 Unix `sigaction` 或 Windows SCM dispatcher；handler 只翻转共享原子 stop token，当前进程直接取得 daemon lease，不经过 background launcher/child。running state、PID projection 与 lease owner 发布后才报告 service ready；stop 由 control loop 转换成 generation-bound `Shutdown` 请求并复用 W1 drain、shutdown evidence、stopped state、PID 删除和 lease release 事务。新发布的 v3 control request 携带活动 lease generation；service-stop 优先消费同代已落盘 shutdown，跨代残留会被隔离且不能停止 successor；显式 v1 请求仍走兼容读取路径。仓库内定向回归覆盖路径空格/转义、缺失/重复 identity、argv/config drift、direct PID/lease identity、活 lease 拒绝第二入口、generation-bound 残留隔离与幂等 token shutdown；真实 Windows SCM、PID1 systemd、macOS launchd 生命周期 transcript 仍明确归 W2-L03 至 L05、W2-L09/L10 |
| W2-L08 | Boot/restart recovery | W1-L10、W2-L07 | 已阻塞 | recovery、lease fencing 与 W2-L07 service identity/no-respawn 前置代码已完成；仍需 EXT-01 上的 crash/reboot、queued-work 恢复和 no-double-worker 验收 |
| W2-L09 | 三平台 destructive harness | W1-L12、W2-L03 至 W2-L08、EXT-01 | 已阻塞 | W2-L08 代码前置已具备，但仍需 EXT-01 boot/recovery 验收及具备提权/reboot、mandatory cleanup 的受控 service host |
...
| W3-L08 | Run-as user isolation | W2-L07、W3-L01、W3-L03 | 验证中 | 代码边界已接线：Unix 非 root 仅允许精确当前 euid/egid，检测到 supplementary groups 时 fail closed；root spawn 前清 supplementary groups、setresgid/setresuid；Linux 额外清 keepcaps、ambient/effective/permitted/inheritable capabilities 并设置 `NO_NEW_PRIVS`。Windows 通过 `LookupAccountNameW` 解析请求 SID，仅允许与 daemon primary token 同 SID 的 same-service-token 继承，未知或不同 SID 在 spawn 前拒绝，当前不支持任意跨账户 token 创建。stdio、MCP stdio、command-backed Skill 显式传播 run-as，HTTP/MCP HTTP/Builtin/Lua/Eventbus/Hardware 与 builtin Skill 对 non-current run-as fail closed，runtime 及 Skill workspace/input 创建前均预检。manifest digest 已升为 v4，并用长度前缀绑定 stdio/MCP/Skill argv 和 MCP transport policy。现有仓库测试、Windows same-SID 与 Unix helper 通过；EXT-01 原生 Linux/macOS/Windows service identity evidence 尚未完成。macOS 显式切换当前未开放，等待受控 no-suid 或 sandbox 方案，`Current` 仍可用；`pre_exec` 会再次复核 supplementary groups，但原生 host 对组状态与 root 创建目录 UID/GID handoff 的 attestation 仍缺。Skill workspace/artifact 已加入 no-follow、owner/regular-file 检查和句柄读取，但跨 UID handoff、目录 ancestor race 与完整 TOCTOU 仍需生产 hardening |
| W3-L09 | Credential vault session | W3-L01、W3-L08 | 验证中 | 已接入 `CredentialVault`/`CredentialSession`/`CredentialSessionLease` trait、显式 `MemoryCredentialVault` 测试实现和生产 `FailClosedCredentialVault` 默认；manifest 只传 `vault://` ref 或 allowlisted env 名，session 按调用获取、注入、释放，release 幂等并记录释放审计。stdio、MCP stdio、Skill 使用 `env_clear()` + 最小环境，HTTP/MCP HTTP 从 session 解析 header 凭据；缺 scope、缺 secret、空 secret、未 allowlist lazy env、process-free provider credentials 均在 spawn/I/O 前拒绝。SecretValue、invocation/config/report、MCP/stream Debug、audit、artifact、stream 与 error 均有脱敏边界；Skill workspace/artifact 使用 no-follow、owner/regular-file 检查、sync 和打开句柄读取，artifact store 也拒绝 symlink/reparse。仓库回归：`cargo test -p eva-adapter --all-targets` 91 passed/7 ignored、`cargo test -p eva-mcp --all-targets` 23/23、`cargo test -p eva-storage --lib artifact_store` 19/19、`cargo test -p eva-config --all-targets` 63/63、相关 Clippy `-D warnings` 与 fmt 通过。仍缺真实 OS secret store/KMS authority、EXT-01 原生 identity/ownership transcript、跨 UID Skill handoff、目录 ancestor race/完整 TOCTOU 和生产 leak-scan evidence，因此保持验证中；B-P0-02 不解除 |
| W3-L10 | Daemon-owned supervisor lifecycle | W1-L11、W3-L05 至 W3-L09 | 验证中 | daemon 与 retrieval worker 现在共享单一 `AdapterRuntime` 和 supervisor；单向 draining gate 以 `provider_draining` 拒绝新 provider 调用，bounded drain 执行 identity-fenced cleanup、snapshot CAS retirement、精确 admission reservation 释放及 pending/resolved replay evidence，并对无 OS identity 的 legacy snapshot 保持 fail-closed。启动 guard 覆盖 runtime 创建后的异常路径，shutdown 先关闭 provider admission，再把共享绝对 deadline 的剩余预算传给 task worker。supervisor、runtime、daemon、worker 回归以及 workspace tests/Clippy/fmt 已通过。W3-L08 原生 identity evidence、W3-L09 OS/KMS 与 ownership/TOCTOU/leak evidence、W3-L11 三平台 production evidence 仍未完成，因此本项尚不能标记为已完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W4-L01 | MCP production transport config | W3-L10 | 验证中 | `eva-mcp` 新增 canonical `streamable_http`（保留 `http` legacy alias）及强类型 endpoint/origin、trust-root、client-auth、redirect、allowed-origin 配置；HTTP(S) URL、IPv6/default port、保留/重复 header、redirect hop 和 public config mutation 均 fail closed，Debug/audit 不泄露 endpoint path/query 或凭据引用。direct manifest loader 与 schema 对未知字段及 stdio/HTTP 联合字段保持严格一致，`AdapterHandle` 无损保存 policy，v4 manifest digest 绑定完整 transport 配置；production HTTPS 缺 trust/origin 拒绝，TLS-only policy 不能用于明文 HTTP。`cargo test -p eva-mcp --all-targets` 28/28、`cargo test -p eva-config --all-targets` 68/68、`cargo test -p eva-adapter --all-targets` 97 passed/7 ignored，workspace tests、Clippy `-D warnings`、fmt 与 schema/docs 检查通过。HTTPS I/O、证书材料读取和 `file:` 相对受控项目根解析归 W4-L02；当前不得使用进程 cwd |
| W4-L02 | HTTPS/TLS connector | W4-L01 | 验证中 | `crates/eva-mcp/src/http_transport.rs` 已接入 rustls、系统/自定义 PEM trust root、证书链、hostname/SAN、SNI、有效期、mTLS client cert/key 和 connect/handshake/read/write 稳定错误分类；`McpJsonRpcClient` 已通过真实 TLS stream 执行 initialize、initialized、tools/list、tools/call。Adapter 每次调用从 `env:` 或已声明 `vault://` 引用构造 TLS material，transport/material 先销毁再恰好 release 一次，secret 不进入 HTTP child env、Debug 或 audit。`file:` 材料仅从 canonical 受控项目根读取并拒绝 traversal、静态 symlink/reparse、Windows ADS、非普通文件与超限文件。`cargo test -p eva-mcp -p eva-adapter -p eva-config --all-targets` 通过（MCP 42/42、Adapter 101 passed/7 ignored、Config 68/68），workspace tests、Clippy `-D warnings`、fmt 与 Windows GNU cross-check 通过。仍缺覆盖 DNS 与所有 I/O 的单调总 deadline、真实阻塞 write/slow-drip evidence、关闭 ancestor replacement TOCTOU 的 Unix `openat`/`openat2` 和 Windows handle traversal/final-path 验证，以及原生 Linux/macOS/Windows evidence；Apple cross-target 因 host 缺 `cc` 未完成 |
| W4-L03 | 完整 HTTP framing | W4-L02 | 已完成 | `crates/eva-mcp/src/json_rpc.rs` 已将 response parser 改为有界 CRLF 状态行/header 与显式 body framing：Content-Length、chunked extensions/trailers、HTTP/1.0 close-delimited、keep-alive、1xx interim、202 notification bodyless、204/205/304 no-body 均有明确语义；拒绝 CL/TE 冲突、重复/非法 header、缺少 framing、非法 chunk/trailer、压缩编码和超限 body，非空 JSON-RPC response 必须是 `application/json`。分片 reader、真实 keep-alive/chunked socket、202/204、oversize、invalid header/body 与无 EOF 等 51 项 MCP 测试通过；session lifecycle、增量 SSE 与真实 abort 已分别由 W4-L04/L05/L06 完成 |
| W4-L04 | Streamable HTTP session | W4-L03 | 已完成 | 新增 `crates/eva-mcp/src/streamable_http.rs` 会话门面，并在 `json_rpc.rs` 的有界 response head 中保留、校验且脱敏 `Mcp-Session-Id`。initialize 请求/响应只从严格 JSON 顶层及直接 `params`/`result` 字段取值，并在单一支持版本协商成功后提交 session；transport 强制 initialize、initialized、ready、closed 阶段，后续 notification/POST/GET/DELETE 自动携带 `MCP-Protocol-Version` 及可选 session ID，manifest 与 runtime 均拒绝调用方伪造控制 header。notification 仅接受空体 202，GET 仅接受 `200 text/event-stream`；404 清空本地状态，session mismatch poison transport 并保留可信 ID 做 DELETE，无效 initialize 的 provisional session 自动清理，stateless shutdown 不发 DELETE 但关闭本地对象。真实 socket/负向回归覆盖完整五请求序列、错误 cleanup、GET header、嵌套字段欺骗、阶段乱序、精确响应形态和 202/204/404/405 shutdown；`eva-mcp` 63/63、`eva-config` 68/68、workspace tests/Clippy/fmt 与 Windows GNU cross-check 通过。GET/POST 的增量 `text/event-stream` reader、事件关联与断流处理已由 W4-L05 完成 |
| W4-L05 | SSE incremental parser | W4-L04 | 已完成 | 新增 `crates/eva-mcp/src/sse.rs`：按任意 byte/UTF-8 分片增量解析 CR/LF/CRLF、BOM、多行 `data`、comment ping、`event`/`id`/`retry` 与 error event，分别约束单行、单 event、单 push 与 pending queue；严格直接 JSON-RPC envelope 将 notification/request/response 分离，嵌套 ID 不参与关联，同 ID peer request 不会命中 response，显式 stream 可保留交错消息。`json_rpc.rs` 将 response head 与 body reader 分离，以 pull-based Content-Length/chunked/close-delimited source 支持 session-bound GET、显式 POST stream 和普通 POST 的透明 SSE response，匹配 ID 后无需 EOF；Debug/audit 不输出 payload 或 opaque request/event/session ID。逐字节 UTF-8、BOM/换行、ping/error、断流、非法 UTF-8、line/event/pending 上限、严格 envelope、三种 framing、chunk/SSE 边界错位、keep-open socket 与 GET/POST session/version header 回归均通过；`eva-mcp` 78/78、package Clippy `-D warnings` 与 fmt 通过。真实 abort/cleanup 已由 W4-L06 完成 |
| W4-L06 | Abort 取消真实 I/O | W4-L05 | 已完成 | 新增独立于 stdio registry 的 `McpStreamableHttpSessionRegistry`，持有 Streamable HTTP session、SSE reader thread、共享底层 socket abort handle、有界非阻塞事件队列、completion receiver 与 `JoinHandle`。abort 先 fence/cancel，再确认所有 socket shutdown 成功，随后 DELETE 最终 session、bounded wait/join reader，全部成功后才移除状态；socket shutdown 失败不会继续 DELETE，DELETE/join 失败保留永久 `CleanupPending` 占位，禁止新 GET/POST/poll 并只允许 cleanup 重试。reader 自然结束、queue empty/disconnected、队列超限主动关 socket、未 ready session cleanup owner 均有回归；真实 HTTP fixture 观察到 socket 先关闭、随后收到带 session/version header 的 DELETE，reader 已 join 且 dangling session/reader 为 0，DELETE 500 可重试。明文/TLS 底层 abort 连续回归、`eva-mcp` 83/83、workspace tests、workspace Clippy `-D warnings`、fmt、Windows GNU cross-check 与 `git diff --check` 通过；完整 TLS registry fixture 与 socket shutdown fault injection 尚未覆盖，daemon ownership 归 W4-L07 |
| W4-L07 | Supervisor-owned MCP lifecycle | W3-L10、W4-L06 | 已阻塞 | W4-L06 真实 abort 已完成；仍等待 W3-L10 完成验证并将 HTTP registry 纳入 daemon supervisor ownership |
| W4-L08 | 真实 MCP server transport | W4-L04、W4-L06 | 可开始 | 直接前置已完成；下一步接入真实受控 server transport，并验证显式 tool 暴露与 handler 前拒绝 |
...
| W8-L04 | Daemon memory/retrieval worker | W8-L03 | 验证中 | 新增强类型、默认禁用的 `runtime.retrieval_worker` 配置并完成字段校验。daemon 以独立 durable schedule 在启动和 control tick 领取到期任务，使用显式 provider、项目 capability registry、最小权限、runtime policy/redaction 和持久 provider process table 构造 adapter-backed host；禁用时稳定 idle，未到期重启不执行，只有 `indexed` 结果才 generation-fenced complete，拒绝/失败保留 lease 供到期重试且不写 durable knowledge。daemon-owned provider lifecycle/drain 已通过共享 runtime 接入并处于 W3-L10 验证中；真实 source 与 durable index commit 仍归 W8-L05，native renew/crash-reclaim 和三平台 process evidence 归 W3-L11/EXT-01 |
