...
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | 新增 `eva-storage::FileSystemProviderAdmissionTable`，以每个 Adapter 的稳定文件锁作为线性化点，持久化 reservation、容量检查、同 session 幂等、完成释放和过期回收；`InMemoryProviderSupervisor` 在 durable process table 模式下执行 reservation，并在 process CAS 失败时补偿释放。新增 `provider_admission::tests::two_processes_have_one_winner_for_capacity_one`，由父测试启动两个真实子进程竞争 `max_concurrency=1`，确认仅一个成功且持久表仅保留一个 reservation；`eva-storage` admission 单测、`eva-adapter`/`eva-storage` 测试和 `-D warnings` Clippy 通过。crash reclaim、长任务 renew 和对应 W0 evidence 尚未完成，需受控进程 harness/evidence 后才能标记已完成 |
