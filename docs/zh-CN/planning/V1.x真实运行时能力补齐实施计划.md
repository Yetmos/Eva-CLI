...
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | 新增 `eva-storage::FileSystemProviderAdmissionTable`，以每个 Adapter 的稳定文件锁作为线性化点，持久化 reservation、容量检查、同 session 幂等、完成释放和过期回收；`InMemoryProviderSupervisor` 在 durable process table 模式下执行 reservation，并在 process CAS 失败时补偿释放。父测试启动两个真实子进程竞争 `max_concurrency=1`，确认仅一个成功且持久表仅保留一个 reservation；新增 reservation ID + session 双身份 fenced renew，只有未过期 owner 可延长 TTL，错误 session 与过期 lease 均拒绝。`eva-storage` admission 单测、`eva-adapter`/`eva-storage` 测试和 `-D warnings` Clippy 通过。supervisor 周期 renew、crash reclaim 与对应 W0 evidence 尚未完成，需受控进程 harness/evidence 后才能标记已完成 |
