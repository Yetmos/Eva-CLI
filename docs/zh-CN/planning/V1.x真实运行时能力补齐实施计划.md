...
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | durable reservation、renew/fence、真实双进程 capacity=1、crash-expiry reclaim 与 stale identity 回归均已完成。新增 CI W3 producer：使用既有 command capture 运行 `provider_admission::tests::`，验证关键场景出现在 stdout，生成 `eva.provider-admission-evidence.v1`，绑定 commit、run/attempt、job/OS/arch executor、UTC timestamp 与 capture SHA-256，并上传三平台 artifact；本地 capture/readback 合同通过。等待新 CI 三平台 artifact 成功后完成 |
