...
| W9-L05 | Credential-free validator run | W9-L02 至 W9-L04 | 验证中 | 新增 `validate-package-manager-metadata.ps1`：严格 UTF-8/size/静态结构检查，并在工具存在时以无 shell argv 调用 `brew audit --strict --formula`、`winget validate --manifest` 或 `apt-ftparchive packages`；工具缺失稳定输出 `unavailable`，`-RequireNativeTool` 非零。自测试覆盖三格式与损坏输入 4/4，并加入三平台 CI contract；本机实际发现 Winget validator 可调用、brew/apt 不可用。仍需 generator 文件落盘入口及三平台 `-RequireNativeTool` evidence 后才能已完成 |
