...
当前共有 123 个逻辑修改：1 个可开始、74 个已阻塞、0 个进行中、7 个验证中、41 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W6 | 11 | 1 | 0 | 7 | 0 | 0 | 3 | 0 | 0 | Open + DEC-02 |
...
| W6-L04 | Reload preflight/fail-safe | W6-L03 | 可开始 | W6-L03 已完成，可消费 batch 构建候选并记录失败 audit |
