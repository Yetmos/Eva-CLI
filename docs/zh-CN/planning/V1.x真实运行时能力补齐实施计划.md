...
当前共有 123 个逻辑修改：6 个可开始、85 个已阻塞、0 个进行中、3 个验证中、29 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W3 | 11 | 1 | 0 | 5 | 0 | 0 | 5 | 0 | 0 | Open + EXT-01 |
| W4 | 10 | 0 | 0 | 10 | 0 | 0 | 0 | 0 | 0 | Open + EXT-03 |
| W5 | 12 | 0 | 0 | 12 | 0 | 0 | 0 | 0 | 0 | Open + EXT-02/EXT-08 |
| W6 | 11 | 1 | 0 | 10 | 0 | 0 | 0 | 0 | 0 | Open + DEC-02 |
| W7 | 12 | 1 | 0 | 11 | 0 | 0 | 0 | 0 | 0 | Open + EXT-04 |
| W8 | 12 | 3 | 0 | 9 | 0 | 0 | 0 | 0 | 0 | Open + EXT-05/EXT-09 |
...
| W3-L06 | Durable restart controller | W3-L02、W3-L05 | 已完成 | `restart.rs` 提供有界 attempt budget、指数 backoff、确定性 jitter 与 24 小时上限；provider process v3 以 `record_version + owner_generation` CAS 持久化 attempts、绝对 due time 及 pending/starting/running/stable/exhausted/failed 状态。stdio、MCP stdio 和进程型 Skill 统一登记真实 PID；成功请求清零预算，长驻 provider 满 30 秒稳定窗口后重置，daemon recovery 跨 generation 保留 attempt/backoff，starting 崩溃不重复消耗预算。crash-loop、output-limit 非重启、generation-change、stable-window 与 recovery 回归均通过；Windows 三 crate 测试、workspace Clippy、fmt、Windows GNU/macOS cross-target checks 通过。native Linux/macOS crash-loop 与 daemon-kill evidence 留给 W3-L11；run-as/vault 与跨进程 admission 仍待 W3-L07 至 W3-L09 |
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | 新增 `eva-storage::FileSystemProviderAdmissionTable`，以每个 Adapter 的稳定文件锁作为线性化点，持久化 reservation、容量检查、同 session 幂等、完成释放和过期回收；`InMemoryProviderSupervisor` 在 durable process table 模式下执行 reservation，并在 process CAS 失败时补偿释放。`eva-storage` admission 单测、`eva-adapter`/`eva-storage` 测试和 `-D warnings` Clippy 通过；真实双进程 contention、crash reclaim、长任务 renew 和对应 W0 evidence 尚未完成，需受控进程 harness 后才能标记已完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 验证中 | 新增 `eva-config::ConfigGeneration`，对已加载且校验通过的项目配置生成稳定环境标识与内容 digest，并拒绝零 generation；同一配置重复计算保持一致。`eva-config` 55 项测试、fmt 与 Clippy 通过；profile/user/env 分层 merge、覆盖冲突诊断和原子 reload 尚未接线 |
...
| W8-L02 | Durable schedule model | W1-L01、W1-L12 | 验证中 | `eva-memory::FileSystemScheduleStore` 持久化 next-run、owner lease、expiry 和 generation，支持 upsert/claim/complete、过期租约接管及重启恢复；具备 per-schedule create-new lock、60s 崩溃残留锁回收和并发 claim 单赢家。本次新增 generation-fenced renew，旧 owner、旧 generation 或已过期 lease 均不能续租。`eva-memory` 34/34、fmt 与 Clippy 通过；尚未接入 daemon 长驻 scheduler 与生产 retention policy |
...
| W8-L07 | Runtime-wide OTel lifecycle | W1-L12 | 验证中 | 新增 runtime-owned `RuntimeObservabilityLifecycle`，统一 start、pipeline access、flush/shutdown 状态门禁和 continuity report，避免各调用点自行创建短生命周期 pipeline；`eva-observability` 22/22、fmt 与 Clippy 通过。当前尚未由 daemon 全局持有，也未覆盖真实 collector 长驻 flush 证据 |
...
| W9-L02 | Homebrew formula | W9-L01 | 已完成 | 新增确定性 `generate_homebrew_formula`，只消费 canonical macOS artifact URL/version/SHA-256，生成 Homebrew Formula 的 install 与 version smoke；重复生成字节一致。定向测试与 `eva-release` Clippy 通过；tap upload 与真实 brew install 属于 W9-L05/W9-L11/W9-L12 |
| W9-L03 | Winget manifests | W9-L01 | 已完成 | 新增确定性 `generate_winget_manifests`，从 canonical Windows artifact 生成 Winget version/installer/defaultLocale 三份 manifest，固定 portable x64、user scope、upgrade behavior、下载 URL 与 uppercase SHA-256。定向测试与 `eva-release` Clippy 通过；winget validate/仓库提交留给 W9-L05/W9-L11 |
| W9-L04 | deb/Apt metadata | W9-L01 | 已完成 | 新增确定性 `generate_apt_metadata`，从 canonical Linux artifact 生成 Debian control 与 Apt Packages stanza，固定 package/version/amd64/installed-size/pool path、upstream URL 和 SHA-256，并拒绝零 installed size。定向测试与 `eva-release` Clippy 通过；dpkg/apt validator、签名与仓库上传分别留给 W9-L05/W9-L09/W9-L11 |
| W9-L05 | Credential-free validator run | W9-L02 至 W9-L04 | 验证中 | `validate-package-manager-metadata.ps1` 严格检查 UTF-8/size/静态结构，并以无 shell argv 调用 brew/winget/apt native validator；缺工具稳定 `unavailable`，`-RequireNativeTool` 非零。新增 `write_package_manager_metadata`，在受控输出根原子生成 Formula、Winget 三 manifest、Apt control/Packages 共六文件；定向测试、Clippy、自测试 4/4 通过。仍需提供 canonical manifest CLI 输入并在三平台对生成文件运行 `-RequireNativeTool` evidence |
