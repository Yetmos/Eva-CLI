...
当前共有 123 个逻辑修改：0 个可开始、66 个已阻塞、0 个进行中、6 个验证中、51 个已完成。这里的“已阻塞”表示严格直接前置或明确的外部验收边界尚未满足，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W3-L08 | Run-as user isolation | W2-L07、W3-L01、W3-L03 | 已阻塞 | service identity 与 process backend 已完成；仍需 EXT-01 受控身份 host 验收 Unix uid/gid 与 Windows service token 隔离 |
| W3-L09 | Credential vault session | W3-L01、W3-L08 | 已阻塞 | W3-L08 run-as 实现与受控 identity host 验证完成后，才能推进 vault session |
