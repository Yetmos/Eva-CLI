...
当前共有 123 个逻辑修改：0 个可开始、77 个已阻塞、1 个进行中、7 个验证中、38 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W6 | 11 | 0 | 0 | 10 | 1 | 0 | 0 | 0 | 0 | Open + DEC-02 |
...
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | `eva-storage::FileSystemProviderAdmissionTable` 持久化 reservation、容量检查、过期回收，并以 reservation ID + session 双身份 fence renew/complete。`AdapterRuntime` 现在为每次受监督 transport 尝试启动独立 renewal owner，以 TTL 三分之一为周期续租，transport 返回时同步停止并 join；续租失败覆盖业务结果并 fail closed，随后仍走 fenced completion。父测试启动两个真实子进程竞争 `max_concurrency=1`，确认仅一个成功；错误 session 与过期 lease 均拒绝。`eva-adapter` 76 passed、6 helper ignored，Clippy、fmt 与 diff-check 通过。真实子进程 reserve 后无清理退出的回归证明过期前 successor 被拒绝、过期后可接管，且旧 identity 不能释放 successor；`eva-storage` 125/125 与 Clippy 通过。剩余仅需把受控环境运行结果封装为绑定 commit/environment/executor/timestamp/digest 的 W0 evidence 后才能标记已完成 |
...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 进行中 | `ConfigGeneration` 已提供稳定环境标识与 digest primitive；新增 `layering.rs` 定义 base/profile/user/environment 固定优先级，按 layer kind 与路径确定性排序，递归合并 mapping、只允许同类型 scalar/sequence 覆盖，并对缺失/重复 base、非 mapping root、非字符串 key 和类型漂移 fail closed；`field_sources` 记录每个最终叶字段的来源层。`eva-config` 59/59、Clippy、fmt 与 diff-check 通过；loader 现按项目内固定路径发现 `config/profiles/<env>.yaml`、`config/eva.user.yaml`、`config/environments/<env>.yaml`，不读取宿主 home 或自由环境变量；合并后使用 base 指定的 schema root 重新执行主 schema 与 typed parse，再枚举 split manifests。默认项目加载、`eva-config` 59/59、Clippy、fmt 与 diff-check 通过；临时项目端到端测试覆盖 profile→user→environment 优先级、merge 类型漂移和合并后 schema enum 拒绝，`eva-config` 61/61 与 Clippy 通过；`eva-core::sha256_digest` 新增无依赖标准 SHA-256，并通过空串/abc 已知向量；storage 复用后 125/125 保持 digest 兼容。下一步以该 primitive 实现 secret-free canonical digest |
