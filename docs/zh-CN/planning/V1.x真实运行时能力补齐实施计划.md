...
当前共有 123 个逻辑修改：0 个可开始、75 个已阻塞、1 个进行中、7 个验证中、40 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W6 | 11 | 0 | 0 | 8 | 1 | 0 | 2 | 0 | 0 | Open + DEC-02 |
...
| W6-L03 | Long-lived config watcher | W6-L02 | 进行中 | 新增 `ConfigWatcher`，以项目 `config/` 递归常规文件 snapshot 检测 YAML/JSON/Lua source，连续变化经静默 debounce 窗口合并为相对路径稳定排序的单 batch；stop channel 立即唤醒轮询线程，`stop_and_join` 幂等，Drop 兜底 join。真实文件 burst/coalescing/no-second-event/shutdown 测试与 runtime Clippy 通过；下一步接 daemon 生命周期与 hot_reload disabled idle |
