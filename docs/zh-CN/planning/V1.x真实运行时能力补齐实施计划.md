...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 进行中 | `ConfigGeneration` 已提供稳定环境标识与 digest primitive；新增 `layering.rs` 定义 base/profile/user/environment 固定优先级，按 layer kind 与路径确定性排序，递归合并 mapping、只允许同类型 scalar/sequence 覆盖，并对缺失/重复 base、非 mapping root、非字符串 key 和类型漂移 fail closed；`field_sources` 记录每个最终叶字段的来源层。`eva-config` 59/59、Clippy、fmt 与 diff-check 通过；loader 现按项目内固定路径发现 `config/profiles/<env>.yaml`、`config/eva.user.yaml`、`config/environments/<env>.yaml`，不读取宿主 home 或自由环境变量；合并后使用 base 指定的 schema root 重新执行主 schema 与 typed parse，再枚举 split manifests。默认项目加载、`eva-config` 59/59、Clippy、fmt 与 diff-check 通过；临时项目端到端测试覆盖 profile→user→environment 优先级、merge 类型漂移和合并后 schema enum 拒绝，`eva-config` 61/61 与 Clippy 通过；下一步实现 secret-free canonical digest |
