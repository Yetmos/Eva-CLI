...
当前共有 123 个逻辑修改：6 个可开始、89 个已阻塞、0 个进行中、3 个验证中、25 个已完成。这里的“已阻塞”表示严格直接前置尚未完成，不表示不能先做只读调研；任何实现差异只有在前置满足后才能进入“进行中”。
...
| W3 | 11 | 1 | 0 | 9 | 0 | 0 | 1 | 0 | 0 | Open + EXT-01 |
...
| W3-L02 | 真实 PID/process record CAS | W1-L01、W3-L01 | 可开始 | W1-L01 与 W3-L01 已完成，等待实现真实 PID/start token/group-job identity、owner generation、attempt/version 和 fenced CAS |
