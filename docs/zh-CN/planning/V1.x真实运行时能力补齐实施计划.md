...
| W8-L02 | Durable schedule model | W1-L01、W1-L12 | 验证中 | `eva-memory::FileSystemScheduleStore` 持久化 next-run、owner lease、expiry 和 generation，支持 upsert/claim/complete、过期租约接管及重启恢复；具备 per-schedule create-new lock、60s 崩溃残留锁回收和并发 claim 单赢家。renew 与 complete 均按 owner + generation fence，旧 owner、旧 generation、已过期 lease 或同名 owner 的旧 execution 均不能续租或完成 successor。`eva-memory` 35/35、fmt 与 Clippy 通过；尚未接入 daemon 长驻 scheduler 与生产 retention policy |
