...
| ID | 修改摘要 | 直接前置 | 状态 | 当前阻塞、下一状态条件或 evidence 引用 |
| --- | --- | --- | --- | --- |
| W6-L01 | Layered config merge/digest | W1-L12 | 进行中 | `ConfigGeneration` 已提供稳定环境标识与 digest primitive；新增 `layering.rs` 定义 base/profile/user/environment 固定优先级，按 layer kind 与路径确定性排序，递归合并 mapping、只允许同类型 scalar/sequence 覆盖，并对缺失/重复 base、非 mapping root、非字符串 key 和类型漂移 fail closed；`field_sources` 记录每个最终叶字段的来源层。`eva-config` 59/59、Clippy、fmt 与 diff-check 通过；下一步接入明确文件发现、合并后 schema/typed parse 和 secret-free canonical digest |
