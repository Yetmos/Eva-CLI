...
| W3-L07 | Cross-process admission | W1-L01、W3-L02 | 验证中 | `eva-storage::FileSystemProviderAdmissionTable` 持久化 reservation、容量检查、过期回收，并以 reservation ID + session 双身份 fence renew/complete。`AdapterRuntime` 现在为每次受监督 transport 尝试启动独立 renewal owner，以 TTL 三分之一为周期续租，transport 返回时同步停止并 join；续租失败覆盖业务结果并 fail closed，随后仍走 fenced completion。父测试启动两个真实子进程竞争 `max_concurrency=1`，确认仅一个成功；错误 session 与过期 lease 均拒绝。`eva-adapter` 76 passed、6 helper ignored，Clippy、fmt 与 diff-check 通过。daemon crash 后真实跨进程 reclaim 与对应 W0 evidence 尚未完成，需受控进程 harness/evidence 后才能标记已完成 |
