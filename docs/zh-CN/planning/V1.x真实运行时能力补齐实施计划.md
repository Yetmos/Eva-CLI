...
| W9-L05 | Credential-free validator run | W9-L02 至 W9-L04 | 验证中 | validator 严格检查 UTF-8/size/静态结构并无 shell 调用 brew/winget/apt；缺工具稳定 `unavailable`，强制模式非零。`eva release package-metadata generate` 现在严格解析 canonical manifest，在受控输出根原子生成 Formula、Winget 三 manifest、Apt control/Packages 六文件，JSON 只返回相对路径；CLI E2E、Clippy、自测试 4/4 通过。剩余三平台对实际生成文件运行 `-RequireNativeTool` 并收集 evidence |
