...
`restart.mode` 支持 `none`、`on_failure`、`always`；`max_attempts` 是首次启动之后的自动重启次数，`backoff_ms` 必须为正。缺少 `supervision` 或 `credentials` 时保持旧 manifest 的 `none/current/空引用` 默认值。自动 restart 和非 `current` identity 只接受进程型 `stdio`、Skill、MCP stdio；HTTP/MCP HTTP 在加载时失败。W3-L06 已为进程型 provider 执行有界 durable restart、指数 backoff/jitter、绝对到期时间和稳定窗口重置；`always` 不会重放已成功的一次性请求。vault 目标必须同时出现在 `permissions.env`，引用会排序并进入 provider manifest digest；W3-L08/W3-L09 仍负责身份切换和 vault fetch。
...
- production Adapter 的敏感 header/字段只能引用已在 `permissions.env` 声明的 `env:NAME`；`credentials.vault` 只保存 `vault://` URI，当前不会取回 secret。
