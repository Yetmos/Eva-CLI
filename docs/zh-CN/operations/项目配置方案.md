...
`restart.mode` 支持 `none`、`on_failure`、`always`；`max_attempts` 是首次启动之后的自动重启次数，`backoff_ms` 必须为正。缺少 `supervision` 或 `credentials` 时保持旧 manifest 的 `none/current/空引用` 默认值。自动 restart 和非 `current` identity 只接受进程型 `stdio`、Skill、MCP stdio；HTTP/MCP HTTP 在加载时失败。vault 目标必须同时出现在 `permissions.env`，引用会排序并进入 provider manifest digest，但 W3-L01 不访问 vault、不切换 OS 身份，也不主动重启进程；这些行为属于后续 W3-L03/W3-L06/W3-L08/W3-L09。
