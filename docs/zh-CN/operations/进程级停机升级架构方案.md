...
| 能力面 | 当前行为 | 不包含 |
| --- | --- | --- |
| `daemon start` | 打开 durable backend、lock/PID/state、recovery scan、observability、memory maintenance 和前台控制循环 | 启动 provider 进程或安装后台服务 |
| daemon control | 通过文件系统 request/response mailbox 执行 status、submit、cancel、drain、reload、shutdown | 网络控制面认证或 PID 活性证明 |
...
默认命令是一次性 smoke：启动、验证边界、shutdown，并在正常退出时删除 lock/PID：
...
`provider_processes_started` 始终为 `false`。status 只根据已保存 state 与 lock/PID 文件是否存在判断，不探测记录的 PID 是否仍存活。异常终止可能留下 `create_new` stale lock，使下一次 start 直接 conflict；当前没有自动 stale-lock 修复或机器重启恢复。
...
- 失败或 stale lock 需要 operator 显式检查，status 不会自动修复。
...
- heartbeat、control epoch、PID reattach 或自动子进程重启；
