...
Eva-CLI 当前提供本地前台 daemon 边界、durable task/recovery 诊断、generation/drain 原语、受 policy 保护的本地 handoff 状态机和 host-bound service lifecycle 命令。它尚不提供 service-manager daemon entrypoint、经过真实验证的开机恢复、两个真实 Runtime 进程或流量切换。
...
| `service install/status/start/stop/restart/uninstall` | 加载 typed 项目配置并调用 host-bound Windows Service、systemd 或 launchd Adapter；Fake 必须显式传 `--dev` | 通过 service entrypoint 启动 daemon，或替代真实生产 host evidence |
...
- 具备一致 service/PID identity 的 daemon service entrypoint，以及经过真实验证的开机/登录恢复；
...
`service_manager.kind: fake` 会在每次 CLI 调用时新建进程内 Adapter，并在项目/服务隔离的 `.eva/service-manager` 开发状态文件中通过排他锁和原子写入恢复、保存状态。这支持可重复的开发 lifecycle smoke，但不是生产 evidence 或平台集成。生产 Adapter 与 CLI 已实现，真实平台 transcript 仍是独立 evidence。
...
- daemon service entrypoint 与 production evidence gate 完成前，在 Eva-CLI 之外维护真实平台 rollback 流程。
