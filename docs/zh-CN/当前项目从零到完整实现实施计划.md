...
| V0.4 | 最小 runtime 闭环 | 待实现 | in-memory EventBus、Scheduler、Agent mailbox、Lua on_event、builtin capability | `examples/basic/` 可端到端跑通 |
...
| `eva-cli` | 完成 V0.3 | `doctor`、`config validate`、`inspect`、`run`、JSON envelope、exit code 已实现 | V0.3 | V0.4 接 `run` 真实事件闭环 |
| `eva-runtime` | 完成 V0.3 | no-op `RuntimeBuilder`、`RuntimeSummary`、`RuntimeServices`、幂等 shutdown 已实现 | V0.3 | V0.4 注入 EventBus/Scheduler/Agent/Lua/capability 服务 |
| `eva-storage` | 骨架 | 边界文件已建 | V0.4 | 以 in-memory 实现开始 |
| `eva-eventbus` | 骨架 | 边界文件已建 | V0.4 | publish/subscribe/dead letter |
| `eva-scheduler` | 骨架 | 边界文件已建 | V0.4 | Topic 路由和 mailbox 投递 |
| `eva-agent` | 骨架 | 边界文件已建 | V0.4 | Agent 生命周期和队列 |
| `eva-lua-host` | 骨架 | 边界文件已建 | V0.4 / V0.5 | Lua sandbox、loader、bindings |
| `eva-capability` | 骨架 | 边界文件已建 | V0.4 | builtin capability 调用 |
| `examples/basic` | 未开始 | 仅有入口说明 | V0.4 | 作为最小闭环示例 |
...
| 模块 | 任务 | 交付 |
| --- | --- | --- |
| `eva-storage` | in-memory event log / state store | 支持 accepted / ack / watermark |
| `eva-eventbus` | publish / subscribe / dead letter | in-memory bus 单元测试 |
| `eva-scheduler` | subscription 和 mailbox 投递 | target / exact / wildcard 路由测试 |
| `eva-agent` | queue、lifecycle、event handler | Agent 能消费一个事件 |
| `eva-lua-host` | 最小 Lua `on_event` | 沙箱 Lua 执行和 `ctx.emit` 占位 |
| `eva-capability` | builtin capability | 一次无副作用 capability 调用 |
| `eva-runtime` | service wiring | builder 装配上述服务 |
| `eva-cli` | `run` 示例 | 端到端示例运行 |
| `examples/basic` | 最小闭环示例 | 事件进入、Agent 处理、capability 返回 |
...
1. 在 `eva-cli` 中实现命令解析和顶层 `eva <command>` 结构。
2. 实现 `config validate --output json` 的结构化诊断。
3. 把 `load_project_config` 的错误输出转换成稳定 CLI 诊断。
4. 将 `EvaError` 的主要错误类型映射到稳定 exit code。
5. 让 `TraceFields` 贯穿 `doctor`、`config validate` 和 `inspect` 输出。
6. 在 `eva-runtime` 中实现 no-op `RuntimeBuilder`、`RuntimeSummary`、`RuntimeServices` 和幂等 shutdown。
7. 让 `inspect runtime` 能读取 runtime summary。
8. 更新 `eva-cli` 与 `eva-runtime` 的 README，使命令示例与真实实现一致。
9. 增加 CLI/runtime 单元测试，并以 `cargo test --workspace` 作为回归门禁。

下一步进入 V0.4：先实现 in-memory storage / EventBus / Scheduler / Agent mailbox，再把 `eva run` 从当前 `Unsupported` 路径推进到 `examples/basic/` 端到端闭环。
