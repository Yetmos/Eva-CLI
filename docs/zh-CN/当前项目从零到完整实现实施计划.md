...
| `eva-cli` | 完成 V1.1 external capability surface | `version`、`doctor`、`config validate`、`inspect`、`run --example basic`、`task status/logs/cancel`、`adapter list/probe`、`mcp list/probe`、`skill list/run`、`discovery scan`、JSON envelope、exit code 已实现 | V1.1 | V1.2 接记忆与知识命令 |
...
下一步进入 V1.2：在不绕过 V1.1 Adapter/MCP/Skill/Discovery 边界的前提下，实现 MemoryService、KnowledgeService 和 ContextBuilder 的受控上下文层。
