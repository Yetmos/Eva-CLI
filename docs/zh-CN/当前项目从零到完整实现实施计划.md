...
| `eva-cli` | 完成 V0.5 | `doctor`、`config validate`、`inspect`、`run --example basic`、`task status/logs/cancel`、JSON envelope、exit code 已实现 | V0.5 | V1.0 接 quickstart/release hardening |
| `eva-runtime` | 完成 V0.5 | no-op runtime、in-memory V0.5 builder、`run_basic`、`TaskReport`、service summary、幂等 shutdown 已实现 | V0.5 | V1.0 接发布稳定化 |
...
| `examples/basic` | 完成 V0.5 | 完整最小 Eva workspace + task/timeout/cancel/replay 示例 | V0.5 | 作为最小闭环和诊断示例 |
...
下一步进入 V1.0：整理 quickstart、安装说明、CI/release 门禁、已知限制和核心示例验收，避免在发布前继续扩大 V1.1 外部能力范围。
