...
| `eva-cli` | 完成 V1.0 core | `version`、`doctor`、`config validate`、`inspect`、`run --example basic`、`task status/logs/cancel`、JSON envelope、exit code 已实现 | V1.0 | V1.1 接外部能力命令 |
...
下一步进入 V1.1：在不破坏 V1.0 core quickstart 的前提下，实现 Adapter、MCP、Skill 和 Discovery 的受控外部能力生态。
