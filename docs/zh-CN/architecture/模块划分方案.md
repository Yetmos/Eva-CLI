...
| `eva-lifecycle` | generation/drain state、upgrade apply lock、supervisor handoff、release-pointer state、rollback、service-manager abstraction | `eva-backup`、`eva-core`、`eva-observability`、`eva-policy` | 真实 systemd/SCM/launchd；当前 service-manager 是 fake/test-oriented 实现 |
