...
daemon start --foreground --no-shutdown-after-smoke | --background
  -> fenced daemon/writer lease 与 durable recovery scan
  -> paused TaskWorkerRuntime + effect ledger + provider/retrieval boundary
  -> ready 发布，随后 task claim/heartbeat/finish CAS
  -> scheduler retry dispatch -> owned replay delivery -> handler -> ACK
  -> config watcher debounce -> candidate generation preflight
  -> prepare/promote/retire generation，或拒绝后保留 active
...
child 已持有 durable PID/lease identity 后发布成功。两种模式都不会主动启动 provider
process。retry tick 会通过 owned task-worker delivery redrive 到期事件，只有绑定 handler
成功后才 ACK replay。持久 loop 还会执行 scheduled memory maintenance/retrieval，并在
启用时运行 config watcher preflight。下节 direct OS service 路径是第三种模式，不经过
background child entrypoint。
