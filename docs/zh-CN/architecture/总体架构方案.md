...
| 组合与运维入口 | Runtime 报告、basic 执行、前台 daemon 控制、诊断、稳定 CLI 输出 | `eva-runtime`、`eva-cli`、根 `eva` binary |
...
当前不支持后台 daemon spawn，daemon 也不会自动启动 provider process。retry tick
会 redrive 到期事件、路由到本轮临时 Scheduler mailbox，并记录 scheduler ack；它不会
为该投递执行 Agent 或 Lua handler。
...
| Generation、handoff、apply lock 与 rollback | `eva-lifecycle` |
| Daemon control file 与 runtime recovery coordination | `eva-runtime` |
...
tracing 集成和显式 OTLP exporter smoke 验证。

以下内容不能描述为已实现的生产能力：

- 后台或 OS-managed daemon；
...
- systemd、Windows SCM 或 launchd handoff；
...
生产 signing/attestation credential、包仓库权限、平台 service-manager 测试环境、
真实或虚拟 hardware fixture，以及生产 database observability sink 仍是 external
blocker。这些项目只被记录，没有被宣称完成。
