> Language: 简体中文
> Canonical source: ../en/rust-lua-eventbus-scheduler.md
...
外接硬件接入由 `外接硬件接入与热插拔架构方案.md` 约束。硬件设备的发现、claim、协议、热插拔和 raw IO 均属于 Rust 托管边界；Lua Agent 只通过硬件 capability 或 `/hardware/**` Topic 参与业务编排。
