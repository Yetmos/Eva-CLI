...
V1.6.5-alpha durable diagnostics 检查点：当前仓库包含可编译 Rust workspace、可执行 CLI
表面、配置校验、V1.0 in-memory basic runtime、本地 task 诊断、V1.1
外部能力诊断、V1.2 记忆/知识上下文、V1.3 硬件绑定计划、V1.4
备份/生命周期诊断、V1.5 发布加固检查、durable EventBus redrive、durable task snapshot、durable audit record、artifact metadata hardening、runtime recovery scanner、redrive checkpoint、recovery audit smoke 和 `inspect durable` backend diagnostics。英文文档位于
[../en/README.md](../en/README.md)，当前承担默认公开入口和稳定英文 slug 的职责；
在英文详案补齐之前，不能用英文摘要覆盖本目录中的详细架构规格。

当前受管理项目版本：`V1.6.5-alpha`（`Cargo.toml` 版本 `1.6.5-alpha`，预发布 Git tag 形式 `v1.6.5-alpha`）。版本规则见 [版本管理方案](release/版本管理方案.md)。
