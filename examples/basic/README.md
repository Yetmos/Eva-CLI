...
This example is the V0.4 minimum runtime loop. It is a complete Eva workspace that can be loaded with:
...
2. `eva-runtime` builds the V0.4 in-memory services.
...
5. `eva-agent` runs a bounded queue and hands the event to `eva-lua-host`.
...
8. The CLI prints traceable text or JSON output with delivery, Agent, Lua, capability, and audit fields.
...
| `config/agents/root-agent/main.lua` | Controlled V0.4 `on_event` script. |
...
The example intentionally uses only in-memory storage and builtin capabilities. It does not start adapters, MCP, hardware, memory, backup, or supervisor services.
