...
Eva-CLI 已经完成 V0.1 到 V1.6.3-alpha 的阶段实现：
...
7. `eva-release` 已实现 V1.6.3 release readiness、security review、performance baseline、migration guide 和 compatibility policy；
8. `eva-storage` 与 `eva-eventbus` 已实现 V1.6 durable backend manifest、migration lock、filesystem EventLog、DurableEventBus、queryable dead-letter store、redrive 基线、durable task snapshot adapter、durable audit sink 和 artifact metadata hardening。
...
版本命名、递增、tag、GitHub Release 和 package 规则见 [版本管理方案](docs/zh-CN/release/版本管理方案.md)。V1.6.3-alpha 的发版内容见 [V1.6.3 Alpha 发布说明](docs/zh-CN/release/V1.6.3-alpha发布说明.md)。
...
V1.6.3-alpha 是源码 alpha 与 durable task/audit/artifact evidence 检查点，不是已经带安装包的完整 runtime 发行版。后续包含 package 支持的 release tag 会发布 GHCR 容器镜像 `ghcr.io/yetmos/eva-cli`；旧 tag 不追溯重新发布。后续工作已经从“是否能落地”收窄为更具体的执行边界：
