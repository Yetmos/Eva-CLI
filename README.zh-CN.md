...
Eva-CLI 当前已经推进到 V1.7.4-alpha Lua hot-reload lifecycle 检查点。仓库内已有可编译 workspace、配置样例、schema、基础契约 crate、项目配置加载、V1.0 in-memory basic runtime、V1.1 外部能力诊断、V1.2 记忆/知识上下文、V1.3 硬件发现/probe/plan-first 绑定、V1.4 的 backup、snapshot、restore plan 和 upgrade check、V1.5 的 release check/security/perf/migration、V1.6.1 durable backend 基线、V1.6.2 durable EventBus redrive 基线、V1.6.3 durable task/audit/artifact evidence 基线、V1.6.4 runtime recovery scanner、ack-safe redrive checkpoint 和 runtime recovered audit evidence、V1.6.5 `inspect durable` 诊断命令和 durable diagnostics release gate、V1.7.1 受限 Lua VM `on_event` 真实执行边界、V1.7.2 只读 Lua context、`ctx.host.log/audit` 和 `ctx.tools.call` 受控 capability 调用边界、V1.7.3 Lua timeout、instruction budget、cancellation 和 memory budget 执行限制，以及 V1.7.4 shadow load 健康检查、scheduler generation route gate、drain evidence 和 rollback audit evidence。

当前受管理项目版本：`V1.7.4-alpha`（`Cargo.toml` 版本 `1.7.4-alpha`，预发布 Git tag 形式 `v1.7.4-alpha`）。版本规则见 [版本管理方案](docs/zh-CN/release/版本管理方案.md)。
...
Eva-CLI 已经完成 V0.1 到 V1.7.4-alpha 的阶段实现：
...
7. `eva-release` 已实现 V1.7.4 release readiness、security review、performance baseline、migration guide、compatibility policy、durable recovery smoke gate、durable diagnostics smoke gate、Lua VM execution gate、Lua host bindings gate、Lua resource limits gate 和 Lua hot reload lifecycle gate；
...
版本命名、递增、tag、GitHub Release 和 package 规则见 [版本管理方案](docs/zh-CN/release/版本管理方案.md)。V1.7.4-alpha 的发版内容见 [V1.7.4 Alpha 发布说明](docs/zh-CN/release/V1.7.4-alpha发布说明.md)，历史 V1.7.3-alpha 内容见 [V1.7.3 Alpha 发布说明](docs/zh-CN/release/V1.7.3-alpha发布说明.md)。
