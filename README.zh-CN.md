...
Eva-CLI 当前已经推进到 V1.6.5-alpha durable diagnostics 检查点。仓库内已有可编译 workspace、配置样例、schema、基础契约 crate、项目配置加载、V1.0 in-memory basic runtime、V1.1 外部能力诊断、V1.2 记忆/知识上下文、V1.3 硬件发现/probe/plan-first 绑定、V1.4 的 backup、snapshot、restore plan 和 upgrade check、V1.5 的 release check/security/perf/migration、V1.6.1 durable backend 基线、V1.6.2 durable EventBus redrive 基线、V1.6.3 durable task/audit/artifact evidence 基线、V1.6.4 runtime recovery scanner、ack-safe redrive checkpoint 和 runtime recovered audit evidence，以及 V1.6.5 `inspect durable` 诊断命令和 durable diagnostics release gate。

当前受管理项目版本：`V1.6.5-alpha`（`Cargo.toml` 版本 `1.6.5-alpha`，预发布 Git tag 形式 `v1.6.5-alpha`）。版本规则见 [版本管理方案](docs/zh-CN/release/版本管理方案.md)。
...
Eva-CLI 已经完成 V0.1 到 V1.6.5-alpha 的阶段实现：
...
7. `eva-release` 已实现 V1.6.5 release readiness、security review、performance baseline、migration guide、compatibility policy、durable recovery smoke gate 和 durable diagnostics smoke gate；
...
版本命名、递增、tag、GitHub Release 和 package 规则见 [版本管理方案](docs/zh-CN/release/版本管理方案.md)。V1.6.5-alpha 的发版内容见 [V1.6.5 Alpha 发布说明](docs/zh-CN/release/V1.6.5-alpha发布说明.md)，历史 V1.6.4-alpha 内容见 [V1.6.4 Alpha 发布说明](docs/zh-CN/release/V1.6.4-alpha发布说明.md)。
...
V1.6.5-alpha 是源码 alpha 与 durable diagnostics evidence 检查点，不是已经带安装包的完整 runtime 发行版。后续包含 package 支持的 release tag 会发布 GHCR 容器镜像 `ghcr.io/yetmos/eva-cli`；旧 tag 不追溯重新发布。后续工作已经从“是否能落地”收窄为更具体的执行边界：
