# Rust + Lua + EventBus Agent 调度架构方案

更新日期：2026-06-08

## 1. 方案定位

本方案基于 `方案.txt` 重新整理，目标不是实现一个中心化的 LangGraph 克隆，而是实现一套 **EventBus 驱动的多 Agent 调度系统**。
...
- Scheduler 订阅 EventBus，根据事件类型、目标 Agent、订阅规则和负载情况，把事件路由到对应 Agent 的私有队列。
...
本方案的核心是事件调度：Agent 通过事件协作，每个 Agent 维护局部状态，Scheduler 负责事件路由，EventBus 负责消息传播。

| 维度 | LangGraph 风格 | 本方案 |
| --- | --- | --- |
| 控制流 | 显式图定义节点和边 | 事件驱动，Agent 订阅和发布事件 |
| 路由方式 | 条件边、节点返回下一步 | Scheduler 根据事件元数据和订阅规则投递 |
| 状态管理 | 中心化共享 State | Agent 局部状态为主，全局状态需额外存储 |
| 扩展方式 | 新增节点并修改图定义 | 新增 Agent 或订阅规则即可接入 |
| 热更新 | 通常更新图或节点实现 | 替换 Lua 脚本或 Agent 实例 |
| 调试方式 | 图执行轨迹和 checkpoint | 事件 trace、Agent 日志、死信队列 |
| 适用场景 | 长流程、确定性强、可图形化表达 | 高解耦、动态协作、异步事件系统 |

两者不冲突。必要时，可以在单个 Agent 内部使用小型图流程处理局部任务，但系统级编排仍由 EventBus + Scheduler 完成。
...
1. 外部系统或 Agent 向 EventBus 发布事件，例如 `user.input`、`task.created`、`tool.finished`。
2. Scheduler 订阅 EventBus。
3. Scheduler 根据事件类型、目标 Agent、订阅表和路由规则选择 Agent。
4. Scheduler 将事件投递到目标 Agent 的私有 `mpsc` 队列。
5. Agent task 从私有队列读取事件。
6. Agent 调用 Lua 处理函数。
7. Lua 逻辑可以调用 Rust 暴露的异步工具函数。
8. Agent 处理完成后可以发布新事件回 EventBus。
...
EventBus 是全局消息通道，不直接执行 Agent 逻辑。

建议第一阶段使用进程内实现：

- `tokio::sync::broadcast`：适合多订阅者事件广播。
- `tokio::sync::mpsc`：适合点对点队列。

推荐组合：

- 全局 EventBus 使用 `broadcast` 发布系统事件。
- Scheduler 到 Agent 使用每个 Agent 独立的 `mpsc` 私有队列。
- 死信队列使用单独 `mpsc` 或持久化表。
...
- 维护 Agent 注册表。
- 维护事件订阅规则。
- 根据事件类型、目标 Agent、标签、优先级、负载选择目标 Agent。
- 将事件投递到 Agent 私有队列。
...
- `id`：事件唯一 ID，用于去重。
- `event_type`：事件类型，例如 `user.input`、`task.completed`。
- `source`：事件来源。
- `target`：可选目标 Agent ID。
...
1. 直接目标路由：事件带 `target = agent_id`，直接投递。
2. 订阅路由：Agent 订阅某类事件，例如 `user.input`。
3. 规则路由：根据 payload、标签、优先级或负载选择 Agent。
...
pub enum RouteRule {
    TargetAgent,
    EventType { event_type: String, agent_ids: Vec<AgentId> },
...
4. 暂停目标 Agent 接收新事件。
5. 等当前事件处理完成或超时取消。
6. 替换 AgentRuntime 中的 Lua State。
7. 恢复事件消费。
8. 失败时回滚到旧 Lua State。
...
所有事件处理日志应记录 `script_version`，方便回溯问题。
...
2. Scheduler 订阅 EventBus。
3. Agent 注册表：`HashMap<AgentId, mpsc::Sender<Event>>`。
4. 每个 Agent 一个 Tokio task。
5. 每个 Agent 一个独立 Lua State。
6. Lua 脚本实现 `on_event(event, ctx)`。
7. Rust 暴露 `ctx.emit` 和一个示例 async tool。
8. Echo Agent demo。
9. 基础 tracing。
10. 超时处理。
11. 死信队列。
...
- Lua 可以通过 `ctx.emit` 发布新事件。
- Lua 可以调用一个 Rust async 工具函数。
- Agent 处理事件有 timeout。
- 失败事件进入死信队列。
- tracing 中能看到完整 correlation 链路。

## 19. 总结

Rust + Lua + EventBus 的合适落点是 **事件驱动 Agent 调度架构**，而不是默认复刻 LangGraph 的中心化有向状态图。

该方案的优势是解耦、并发、热更新和动态扩展；代价是事件一致性、状态管理和调试复杂度更高。
