...
- Integration path：CLI 发布 `/user/input`，示例 Agent 发布 `/agent/reply`。
...
该方案的关键是先明确 Topic、Agent 边界、状态和可靠性语义，再根据运行形态选择进程内或持久化 EventBus 后端。
