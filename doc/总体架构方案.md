...
- `README.md`：定义 `doc/` 目录阅读顺序、文档职责和核心边界。
...
```text
 ...
 config/agents/**/main.lua                ~/.codex/skills/*
      ...
      |             |             |             |             |             |                 |
      v             v             v             v             v             v                 v
 BuiltinAdapter  StdioAdapter  HttpAdapter  EventBusAdapter  McpAdapter  HardwareAdapter  Future
      |             |             |             |             |             |
      v             v             v             v             v             v
 Rust tools      Codex CLI     Claude API    Internal Agent   MCP server    外接硬件
...
4. **扩展层**：AdapterRegistry、AdapterRouter、MCP client/server、HardwareAdapter、外部 provider。
...
- `AdapterManifest`、`AgentManifest`、`CapabilityManifest`、MCP policy 和 SkillAdapter manifest 需要落成 JSON Schema。
...
EvaLauncher-CLI 的总体架构应落在 **Rust 托管运行时 + AgentDiscoveryService + Lua 热更新 Agent / Capability + Topic EventBus + 动态 Adapter + MCP 双向集成 + OS service manager / Supervisor / Runtime 高可用升级恢复**。
