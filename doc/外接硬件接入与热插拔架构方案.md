...
本文定义 EvaLauncher-CLI 如何接入外接硬件，并支持设备插入、拔出、重连、驱动绑定、命令调用、数据上报和运行时恢复。
...
EvaLauncher-CLI 的外接硬件接入应落在 **Rust 托管硬件边界 + HardwareDiscoveryService + DeviceRegistry + DriverBinding + HardwareAdapterRuntime + AdapterRegistry + Topic EventBus** 的组合上。
